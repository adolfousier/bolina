...
validated here is the §8 capability half: the Grant verifier (BE-GRANT-03) and its verification-call boundary (BE-GRANT-03b; the 03c seal was superseded by removal in round 4). §7 (attestation) has no code in this slice and was not red-teamed, so
...
| Mutation kill on the Grant verifier | **12/12 mutants killed**, covering 7/7 modelled BE-GRANT-03 checks plus the BE-GRANT-03b callback property. `tools/mutation-test.py` v4: the modelled-check set is derived from `SPEC.md` at run time (the enumerated 0-11 list and the conformance sentence), not stated by the harness. Two WRONG-CONSTANT, three WRONG-FIELD, three WRONG-OPERATOR, one WRONG-LOGIC, and three callback mutants (absence, before-expiry, before-ledger); full test suite rebuilt per mutant. The 5 unmodelled checks (3, 4, 6, 7, 8) need a certificate store and a pending-intent table the slice defers. |
