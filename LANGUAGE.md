...
| Exit-point coverage (SPEC §11.6) | **17/17 enumerated parser exit points reached** over a bounded run of 4,000,000 inputs (24,000,000 parser calls: six parse entry points per input), zero unreached. Measured by hand-instrumented counters (`src/coverage.zig`), reproduced with `zig build coverage -Dcoverage -Dfuzz-budget=4000000` (re-measured 2026-08-06 after the round-4 parser extensions). Native toolchain coverage has no script-readable output on this toolchain, so manual instrumentation is the measurement, not a proxy (residual in `THREAT-MODEL.md` §4.6). Corpus: 6 seeds (envelope, grant, intent, span, effect, claim from `test/vectors.json`), 4 mutation operators (bit flip, byte overwrite, truncate, saturate), 40% mutated-seed / 60% fully-random, 4096-byte input cap. |
| Mutation kill | **20/20 mutants killed**: 12 on the Grant verifier (7/7 modelled BE-GRANT-03 checks plus the BE-GRANT-03b callback) and 8 on the attestation layer (8/8 section-7 properties). `tools/mutation-test.py` v5: both check sets are derived from `SPEC.md` at run time — the grant set from the enumerated 0-11 list and conformance sentence, the evidence set from the §7 tables and BE-EVID markers (ceiling integers, the method_id->class table, BE-EVID-02/03/05/05a/09/09b), not stated by the harness. Grant mutants: two WRONG-CONSTANT, three WRONG-FIELD, three WRONG-OPERATOR, one WRONG-LOGIC, three callback (absence, before-expiry, before-ledger). Evidence mutants: ceiling and class-table constants, the min() operator, the subject-match guard, the supersession conjunct and the DAG supersession operator, and the three-state tail plus the non-Effect dropout. The 5 unmodelled grant checks (3, 4, 6, 7, 8) need a certificate store and a pending-intent table the slice defers. |
...
bounds checking, against vector-seeded mutations and raw random bytes. The parser is 486 lines,
readable in one sitting, allocates nothing (BE-WIRE-01, O3), and the Grant verifier reached 12/12
mutant kill over the modelled checks and the callback property with plain tooling. The coverage gate (SPEC §11.6) now reports
17/17 exit points and the corpus, where the first merge reported neither. **Residual:** the coverage is
...
- **O2 — Fuzzing is a merge gate, not a milestone.** SPEC §11.6 is promoted: no change touching
  ...
  *Honest record: the Round 3 merge cycle violated the "reporting coverage and corpus" half of this
  ...
  hand-instrumented branch counters (`src/coverage.zig`) reported 11/11 branches and the corpus for that cycle (re-measured 17/17 on 2026-08-06 after the round-4 parser extensions), the
