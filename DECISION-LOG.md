...
## D-062 — 2026-08-12 — The dispatch consumed seam goes durable: hook carries the commit context, tombstone after the effect, orphans surfaced at init

The seam rewrite of PHASE-D-ESTIMATE scope item 2 (dispatch.zig consumed seam) and the restart shape of scope item 3, resolved against D-061's row format and the M10 hook discipline before the code changed.

**Ruling 1 — the check-11 hook signature carries the durable-commit context: `fn (grant_id: []const u8, not_after_ms: u64, now_ms: u64) bool`.** D-061 ruling 1's commit row is `0x01 | grant_id | u64 expiry_ms`, so a durable commit needs the grant's expiry; `commitConsumed` also runs the eager prune of BE-EXEC-01, which needs the clock. Both live in `verifyGrantThen`'s scope (`grant.not_after`, `ctx.now_ms`), so the hook receives all three explicitly rather than reading staged module state. The hook contract stays a bool: true = already consumed (refuse). A commit that fails (resource exhaustion) returns true as well — the refusal surfaces as AlreadyConsumed, fail-safe: an uncommitted grant is unspent, so the effect does not run.

**Ruling 2 — no initialized ledger refuses every grant.** The D-059 stand-in registry worked without setup; the durable seam does not. `consumedHook` with `durable_ledger == null` returns true (refuse), never false: without the durable commit the effect must not run (BE-GRANT-01). Every dispatch test that drives a grant to check 11 now initializes a ledger file under /tmp via `initDurableLedger`; the two grants-to-check-11 tests carry the seam state explicitly.

**Ruling 3 — the tombstone lands in dispatchGrant after `verifyGrantThen` returns, and a failed tombstone write is not a dispatch failure.** Check 11 is the last check before the effect, so a successful return from `verifyGrantThen` means the effect fired; dispatch then appends the published tombstone. If that append fails (disk error), the grant stays consumed and the next recovery re-emits it as an orphan (at-least-once interrupted, D-061's fail-safe), so an executed effect is never reported as an error to the caller.

**Ruling 4 — orphans are surfaced at init as a copied list; the daemon publishes, then tombstones.** `initDurableLedger(io, path, orphan_out)` recovers and copies the orphan grant_ids out, because `Recovery` borrows the ledger's internal buffer and `tombstoneOrphan` mutates the ledger — the caller must hold its own list while it publishes one interrupted Effect per orphan (BE-GRANT-01a) before tombstoning each. The interrupted-Effect publish itself is daemon wiring (it needs channel/session context to construct the Effect message) and stays on the daemon milestone side of the seam: `main.zig` is still the DECLARED skeleton, so the restart path lands here as unit-tested seam surface, not daemon startup code.

**Ruling 5 — BE-GRANT-04 needs no durable code: pending approvals expire by construction.** The intent table is memory-only and nothing in the ledger ever records a pending approval, so a restart starts with zero pending intents and a grant for a previously-pending intent refuses at the intent match. The DAEMON_D test pins this: admit, restart over the same durable log, grant refuses NoPendingIntent.

Rejected alternatives: (1) staged module state (dispatch writes grant_id + not_before before calling verify, the hook reads it) — keeps the old signature but couples the hook to implicit setup a future second grant path could break; explicit parameters cost one signature change in two test stubs and one call site; (2) keeping the in-memory registry as a no-ledger fallback — two consumed mechanisms with different durability semantics is exactly the ambiguity the seam rewrite exists to remove; (3) tombstoning inside verify.zig — the verify frame stays ledger-agnostic (M10 shape: it calls injected hooks and knows nothing about the store), and the tombstone belongs to the dispatch seam that owns the ledger's lifecycle.

Reversible: the hook signature reverts to `fn ([]const u8) bool`, dispatch returns to the D-059 registry (git), the four DAEMON_D tests revert; M1 is untouched (no new markers). What would reopen it: a check-11 consumer that cannot know the grant's expiry at hook time (then the row format, not the signature, changes), or a daemon design where the effect publish is asynchronous and the tombstone needs an in-flight marker (the third row type D-061 designed against).
