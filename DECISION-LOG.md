...
**Decision 5 — the three remaining unreached exits are named, not rounded away.** `data_payload_oversize` needs a type-4 packet payload above 1384 bytes, `cert_ca_count_zero` needs a certificate ending exactly at a zeroed count byte, `bind_cert_len_zero` needs a 66-byte binding message with a zero-length certificate. Each needs a shape the corpus never constructs; none is a parser path that does not exist. They are printed on the gate row every run, so 69/72 stays legible as a reach, and closing them means targeted seeds, which is a decision for whoever wants the last three rather than something to slip in here.
