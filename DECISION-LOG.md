...
**Ruling 3 - G3, sustained adversarial soak.** At least 24h of continuous chaos + differential soak on dedicated hardware with zero unexplained failures. This discharges the deferred continuous component of SPEC 11.6 and inherits what 11.6 already demands of itself: parser coverage REPORTED (R1 - the d089-d091 defence gates running un-wired from the exit code until found is the internal proof that measuring the harness happens at home too), measured config = shipped config (R4, ReleaseSafe). "Zero unexplained failures" is already R3 and stands as written.

**Ruling 4 - feature freeze is part of the line.** Explicitly post-1.0, nothing may join scope after this entry: C ABI, WASM target, multi-CA hierarchy, Utterance path (RED-TEAM-09 F1) - AND every finding accepted-with-name during disposition (example: 64-bit `scope_id` held with T11 as the argument). Accepted limitations freeze WITH the features; neither gets "improved" during the final stretch. The line stays fixed so it cannot recede into a moving horizon.
