...
Declared bounds (SPEC v0.5.0): 64 stored packets per recipient; 4 MiB aggregate per recipient; stored body at most 2048 bytes; TTL 120 seconds (one rekey interval, BE-TR-02), purged lazily at store and drain against the caller's clock (house pattern: no timer thread before the daemon milestone); 1024 packet slots globally. The TTL is bounded by the session-key lifetime: stored bodies are encrypted under the sender-recipient session key, and BE-TR-02 requires session keys to be replaced every 120 seconds; a longer TTL would retain ciphertext under a key past its mandated rotation. Quota exhaustion refuses the store, surfaces a counter, and never blocks live forwarding.
...
## D-060 — 2026-08-11 — Phase C serve-loop: pre-auth parse layer, endpoint map lives in the daemon glue, drain-at-registration bounded by the quota

Three design questions raised by PHASE-C-ESTIMATE.md, resolved against the SPEC text and the committed source before any code.

**Ruling 1 — the parse layer for type 5/6 is pre-authentication on the raw datagram; session state is an acceptance gate, not a parse gate.** BE-SURF-01 names the relay route header and registration among the exactly-three structures parsed from unauthenticated input, and section 5.2a repeats it ("both fixed-size and parsed before authentication"). Section 5.2a's sentence "a relay receives type 5 from a node that has already established a Noise session with it" is an acceptance condition on the forward path: the `sender_index` MUST correspond to an established session at the relay. It is not a parse layer, because BE-MESH-02 forbids the relay from holding key material: a route header that required decryption could not be parsed by a relay at all. Consequence for the serve-loop: classify on the raw datagram's first type byte (1/2/3 to the handshake machinery, 5 to the forward path, 6 to registration, anything else dropped with no service), parse type 5/6 with the bound relay.zig parsers over fixed-size slices, then gate acceptance on session and registration state.

**Ruling 2 — the index-to-endpoint map is owned by the serve-loop (relay_serve.zig), not by session.zig or the listener sub-unit.** Source facts measured this turn: session.zig's Session and SessionTable carry indexes, keys, and transcript hash, and no endpoint; handshake.zig's committed session struct carries none either; listener.zig's recv discards the source address, and the listener sub-unit stands at 246/250 with no room for a source-capturing variant under its frozen cap. The precedent exists: handshake.zig declares its own flat libc sendto outside the listener sub-unit, and the tests declare their own extern set. relay_serve.zig therefore carries its own recvfrom (capturing src_addr) and sendto externs over the Listener's fd, and owns an index-to-sockaddr map populated at exactly two points: session commit during handshake (the sockaddr of the datagram that completed the session), and accepted type-6 registration (the registration's source address is the client's delivery endpoint).

**Ruling 3 — the drain batch is the whole queue at registration, because the declared quota IS the bound.** Registrations are one-shot per session (section 5.2a, BE-MESH-04 note: re-registration is not supported), so nothing after an accepted registration can trigger a deferred drain: a partial drain would strand stored packets until TTL expiry. The per-recipient quota (64 packets, relay_store.MAX_PER_RECIPIENT) bounds the worst-case batch by construction. The serve-loop passes a 64-slot out buffer to drainFor and sends every drained packet before handling the next datagram, which also honours drainFor's borrow contract (bodies alias engine slot storage until the next store()). Worst-case send work per registration: 64 x (20 + 2048) bytes, about 131 KB, single-threaded, bounded, no timer.

**The forward decision table that follows:** type 5 accepted, recipient entry exists, endpoint known: sendTo (live path). Entry exists, endpoint unknown: storeDeferred (the engine re-resolves overlay_addr through the table). No entry: drop with no service (BE-MESH-04 extended to storage by D-058). A sendto failure on the live path drops the packet: UDP semantics, no retry queue, since a retry queue would be a second undeclared storage layer.

Rejected alternatives: (1) parse type 5/6 after session decryption: contradicts BE-MESH-02 and BE-SURF-01; (2) extend listener.zig with a source-capturing recv: breaks the frozen listener sub-unit cap for glue that belongs to the daemon; (3) partial drain with lazy continuation: one-shot registration leaves nothing to continue it; (4) put the endpoint map in session.zig: drags daemon glue into the post-auth session-state sub-unit and couples protocol session state to UDP transport.

Reversible: relay_serve.zig and its tests revert, the SPEC BE-EXEC-04 declaration and non-surface placement revert, the ratchet returns to 112. What would reopen it: a demonstration that type-5 acceptance must not require an established sender session (that gate is the section 5.2a sentence; removing it is a SPEC edit, not an implementation choice), a source-address capture that needs less than flat libc externs on this toolchain (Zig 0.16's std.Io.net requires an event-loop runtime; if the daemon adopts one, the externs take the runtime's shape), or evidence that the registration drain blocks live forwarding beyond its 131 KB worst case (then batching returns, keyed by overlay_addr, draining per datagram).

## D-059 — 2026-08-11 — Phase A dispatch core: post-admission routing over the bound machines, hooks for the seams the daemon supplies

Phase A of DAEMON-ESTIMATE.md falsifies the integration seams: dispatch drives the bound state machines over parsed envelopes, in memory, zero sockets. Four rulings. First, dispatch is post-admission by contract: envelope admission (sequence window, cert chain, role map) stays the caller's seam and lands with the listener in phase B; dispatch's entry gate is verifyEnvelope, which is the BE-ENV-02 envelope signature check (corrected from the estimate's "structural gate" reading: the signature gate belongs to dispatch, admission state stays with the caller). Second, intent, grant, and refusal run their full machines (resolveAndAdmit, verifyGrantThen, verifyRefusalThen); utterance passes through untouched; control and effect are routing-verified only, their full state wiring awaiting session and channel state in phase B. Third, seams the daemon supplies are injected as bare function hooks (M10 shape): execute_effect (single effect call site), cert_for_sender (session-state lookup), on_rejected; the consumed-grant hook (BE-GRANT-01) is a module check-and-set registry in phase A, replaced by the durable ledger commit in phase B. Dispatch owns the sender-and-action seam records because intent.Entry carries neither, bounded at 512 action bytes for phase A (phase B sizes from the session MTU). Fourth, clock is caller-supplied now_ms (house pattern); the executor identity (own_pubkey, own_cert, trusted_ca_keys) is construction state, sourced from session establishment in phase B. The EXECUTING transition lands after a successful grant verify frame; dispatch is single-threaded in memory, so the match-to-transition frame race BE-GRANT-03a names does not exist until the phase-B listener owns the frame.

Rejected alternatives: (1) dispatch runs admission, which couples phase A to ledger and cert state that belongs to the listener seam and re-tests machines that already carry their own binding tests; (2) effect handler as a returned capability, which violates BE-GRANT-03b (nothing leaves the verify frame); (3) control and effect deep wiring now, which requires channel state that does not exist until phase B and would falsify nothing real; (4) a structural-only entry gate, which misreads verifyEnvelope and would skip the BE-ENV-02 signature the SPEC puts on every envelope.

Reversible: dispatch.zig and dispatch_test.zig revert, tests.zig registration reverts, no ratchet moved (dispatch binds no new marker; it exercises the 109 already bound). What would reopen it: a phase B finding that admission cannot stay the caller's seam without duplicating state (admission moves inward and the dispatch signature changes), a grant-replay vector through the check-and-set stand-in (the registry gains atomic commit semantics before phase B), or an action larger than the session MTU phase B declares (seam bound repriced).

## D-061 — 2026-08-11 — Phase D persistence: two-phase durable ledger, crash-residue recovery, and the BE-LEDGER / BE-GRANT disambiguation

The four design questions raised by PHASE-D-ESTIMATE.md, resolved against the SPEC text and the committed source before any ledger code.

**Ruling 0 — disambiguation, correcting the task brief.** The brief named "BE-LEDGER-01/02/03" as the durable-persistence markers to declare. They are not: BE-LEDGER-01/02/03 are the DAG ledger (the member's hash chain — reject unknown-parent envelopes at line 1649, hashes-not-plaintext at 1653, every Grant and Effect appears at 1658), already declared and already bound. The durable consumed-grant mechanism is governed by **BE-GRANT-01** (durable commit before the effect) and **BE-GRANT-01a** (committed-but-unpublished publishes an `interrupted` Effect on restart), both already declared in §2. Phase D therefore declares exactly one new marker — **BE-EXEC-01** (daemon lifecycle: one process, no fork-per-session, bounded resources, restart semantics) — and *implements* BE-GRANT-01/01a, BE-GRANT-04 (pending approvals EXPIRE on restart), and BE-REV-02 (revocation persists) durably rather than re-declaring them. No DAG marker is touched.

**Ruling 1 — the on-disk format is a two-phase append log: `commit` row then `published` tombstone row.** SPEC §2 is explicit that check 11 ("grant_id is absent from the consumed ledger, and is durably committed before proceeding") is "the only check that performs I/O, and it runs last by obligation, not by accident," and BE-GRANT-03b states the verify routine "commits the ledger (check 11), and [invokes execution]." So a committed grant_id necessarily passes through a window between the fsync and the effect publish, and BE-GRANT-01a needs to distinguish "committed, effect in flight" from "committed, effect done." The format is: at check 11 the engine appends a `commit` row carrying the grant_id and writes through with an fsync barrier (the only fsync on the commit path); when the effect returns, the engine appends a `published` row for the same grant_id and writes through again. Revocations are single `revoke` rows, never tombstoned (BE-REV-02: a revocation never expires within the certificate's life). Recovery is a single forward scan: pair each `commit` with its matching `published`; an orphan `commit` (no later `published`) is a grant whose effect never completed, which BE-GRANT-01a turns into one `interrupted` Effect plus a synthesized `published` tombstone, so a second recovery is a no-op for that id (recovery is idempotent). This matches the SPEC's "committed before attempted" wording directly and makes recovery a pure function of the file's contents, with no in-memory "published set" to rebuild and lose.

**Ruling 2 — the crash-injection harness is direct file-state construction, not fork-and-kill.** Ruling 1's two-phase format is precisely what makes a crash observable without real process death: "crash residue" is a concrete file state — a `commit` row present, its `published` row absent — that the test writes by hand into a scratch log file and then feeds to `recover()`. No fork, no signals, no killing, no dependence on timing between fsync and publish. The test asserts the `interrupted` Effect fires exactly once and the grant_id is marked consumed and not retried. A real fork-and-kill would test the operating system, not the protocol; this tests the property. (The in-process-abort and log-truncation alternatives from the estimate both collapse into "construct the residue file," which is strictly cheaper and fully deterministic.)

**Ruling 3 — the fsync barrier is observable and is folded into T1.** A write followed by `std.fs.File.sync()` followed by a read-back on the same path returns the committed row on this toolchain; the test (T1) reads the log file after `commitConsumed` returns and before the effect fires, asserting the row is present. No separate probe program is needed.

**Ruling 4 — pruning is sound against "survives redeployment."** BE-GRANT-01 demands the ledger "survive process restart, crash, and redeployment," and `pruneExpired` drops grant_ids past their validity window. There is no conflict: check 10 (expiry) is enumerated before check 11 (the ledger) and is an independent test, so a pruned-and-now-expired grant_id replayed after redeployment is refused at check 10 before the ledger is ever consulted — it cannot be "un-spent" because the grant is also expired. Revocations are never pruned. Pruning touches only rows already past their validity window, never a live one.

Rejected alternatives: (1) single `commit` row plus a separate in-memory published set rebuilt by replaying effects on restart — the rebuild must re-run effects to know what was published, which both re-executes side effects and loses the record on the very crash the ledger exists to survive; (2) SQLite/sled/lmdb embedded store — forbidden by BE-DEP-01 (`build.zig.zon .dependencies = .{}`), and an append log over `std.fs` is the minimal auditable structure the project's determinism posture demands; (3) fork-and-kill crash injection — tests the OS scheduler, not the recovery invariant, and is non-deterministic; (4) committing pending approvals to the durable store — violates BE-GRANT-04 (pending is memory-only so a restart revokes it) and breaks the fail-safe asymmetry the SPEC states at line 1343.

Reversible: the SPEC v0.3.8-draft edit (BE-EXEC-01 promotion, `src/grant_ledger.zig` non-surface placement) reverts, BE-EXEC-01 returns to reserved, the ratchet returns to 113. What would reopen it: a demonstration that a single forward scan cannot infer unpublished grants from orphan commits (which would force a third row type — an explicit in-flight marker — but the two-phase design exists to avoid exactly that), a platform where the post-fsync read-back in T1 does not return the committed row (then the barrier semantics need a separate audit, but the format stands), or a SPEC change making pending approvals durable (which would invert BE-GRANT-04 and require a separate store path). The file is named `grant_ledger.zig`, not `ledger.zig`, because `src/ledger.zig` is the already-placed DAG hash ledger (BE-LEDGER-02/03); naming the new unit `ledger.zig` would shadow it.

## D-062 — 2026-08-12 — Phase D wiring: the check-11 seam widened, the module-level ledger slot, and the fail-safe refusal contract

The five design questions raised by wiring `src/grant_ledger.zig` (D-061) into the dispatch core, resolved against the committed source before the wiring landed.

**Ruling 1 — the check-11 seam is widened to carry `not_after` and `now`.** `verify.GrantContext.already_consumed` moves from `fn(grant_id) bool` to `fn(grant_id, not_after_ms, now_ms) bool`, and `verifyGrantThen` calls it with `(grant.grant_id, grant.not_after, ctx.now_ms)`. The durable commit row's format (D-061 ruling 1) stores the grant's `not_after` so `pruneExpired` can drop grants past their window, and the prune clock is the verify clock (`ctx.now_ms`); both values must therefore reach the hook at the only I/O step. A module-level mirror of `now_ms`/`not_after` set per grant would be a stale-state hazard the instant a second grant is in flight, so the values ride the arguments. The bool contract is unchanged: true = already consumed (refuse), false = newly committed (proceed to the effect).

**Ruling 2 — the ledger lives in a module-level active slot, not a Dispatch field.** The hook is a bare function pointer (M10 shape) and Zig closures cannot capture an instance, so `dispatch.zig` holds `durable_ledger: ?grant_ledger.GrantLedger` at module scope and `consumedHook` reads it. This mirrors the D-059 stand-in it replaces and is safe under BE-EXEC-01: one process, so `dispatchGrant` never runs concurrently against itself. The daemon initializes the slot exactly once at startup via `initDurableLedger`, which also runs recovery and hands the caller the orphan list.

**Ruling 3 — no ledger, or a failed commit, refuses the grant (fail-safe).** `consumedHook` returns true when `durable_ledger` is null: without the durable commit the effect must not run (BE-GRANT-01). A `commitConsumed` that errors (ResourceExhausted) likewise returns true: an uncommitted grant is unspent, so the effect does not run. The refusal surfaces through the hook's single bool as `AlreadyConsumed` at the verify layer: the grant is refused, not retried, and the caller learns only that it did not execute — the fail-safe outcome BE-GRANT-01a names for the unknown-outcome case. Lying about the reason (spent vs commit-failed) is the deliberate trade-off of keeping the verify boundary free of ledger internals.

**Ruling 4 — the tombstone is appended after the effect, and a failed tombstone is swallowed.** `dispatchGrant` calls `markPublished(gid)` after `verifyGrantThen` returns: the effect ran, so the grant is published. A failed tombstone write is not raised: the grant stays consumed, and the next recovery re-emits it as an orphan (at-least-once, BE-GRANT-01a), so an executed effect is never reported as a dispatch error. The tombstone exists to keep recovery idempotent, never to gate the effect.

**Ruling 5 — the recovery seam copies orphans out before the caller tombstones.** `initDurableLedger` copies `Recovery.orphans` into a caller buffer because that slice borrows the ledger's internal buffer and `tombstoneOrphan` mutates the same ledger; the caller holds its own copy while it publishes the interrupted Effect for each orphan, then tombstones. This is the direct-file-state crash-injection shape of D-061 ruling 2: the test writes a commit row whose published row never landed and asserts recovery surfaces exactly one orphan, which one tombstone then retires.

**Ruling 6 — BE-GRANT-04 needs no durable code: pending approvals expire by construction.** The intent table is memory-only and nothing in the ledger ever records a pending approval, so a restart starts with zero pending intents and a grant for a previously-pending intent refuses at the intent match (NoPendingIntent). The DAEMON_D test pins this over a restart. This is why Phase D declares no BE-GRANT-04 durable marker: it is satisfied by absence, not by code.
Rejected alternatives: (1) widen the hook to return an enum (fresh / committed / refused) instead of a bool — adds a type the verify layer would import, breaking the M10 bare-pointer shape and the verify/dispatch boundary it exists to enforce; (2) make the hook a method on the Dispatch instance — impossible, Zig function pointers cannot capture `self`; (3) raise the commit failure as a distinct dispatch error — would widen verify's error union and let an uncommitted grant retry, violating BE-GRANT-01 single-shot; (4) tombstone before the effect — inverts the two-phase order of D-061 ruling 1 and makes a post-tombstone crash lose the grant entirely.

Reversible: revert the dispatch.zig / verify.zig / verify_test.zig / dispatch_test.zig changes; the check-11 seam returns to `fn(grant_id) bool` and the D-059 in-memory `consumed_registry` returns. No M1 marker changes (BE-EXEC-01 was bound at D-061 / commit 89fcad5, M1 114/114; BE-GRANT-01/01a were already declared and are now implemented durably rather than re-declared). What would reopen it: a need to distinguish "refused because spent" from "refused because the commit failed" at the verify layer (ruling 3's lie becomes a liability and the hook must return an enum), or concurrency over `dispatchGrant` (the module-level slot becomes a race and the hook must carry the instance — but BE-EXEC-01 forbids that).

## D-063 — 2026-08-12 — pruneExpired crash-safety: atomic rename replaces in-place truncate (BE-GRANT-01)

The model-checking brief written for SPEC section 11.4 (TLA+/Alloy over the section 8 state machine) predicted a crash window in `pruneExpired`; the prediction was confirmed empirically before the model was built. This is the fix.

**The bug.** `pruneExpired` compacts the log on every `commitConsumed` (it is a hot path, not a rare maintenance step). The original scheme rewrote the live file in place: `setLength(0)` -> `writePositionalAll(survivors)` -> `sync()`. That sequence is not atomic. A power loss between `setLength(0)` and the survivor write + fsync leaves the live file empty. On restart, `recover()` scans that empty log and rebuilds an empty consumed set; a grant that was durably committed and still within its validity window is then absent from the ledger, so check 11 passes and the effect re-executes. That is a direct violation of BE-GRANT-01: a durably committed grant is un-spent by a crash during compaction.

**Confirmation.** A deterministic crash-injection test in the D-061 ruling 2 shape (direct file-state construction, not fork-and-kill): the test writes a committed-but-valid grant, then constructs by hand the exact residue the broken prune leaves after `setLength(0)` and before the write — an empty live file — and asserts the consumed set after `recover()`. Against the old code that test failed (360th of 360), proving the bug is reachable, not theoretical.

**The fix.** Write-to-temp -> fsync-temp -> atomic rename(temp, live) -> reopen live -> fsync. POSIX `rename` is atomic within a single directory, so the live path always resolves to either the previous complete log or the new complete log, never an empty one. A crash during the temp write leaves the old log untouched (temp is discarded on reopen); a crash during or after the rename leaves old or new, both complete. The stale temp from an aborted prune is deleted at the top of every run (idempotent cleanup, `deleteFile` errors swallowed).

**Ruling 1 — the safety invariant holds without a directory fsync.** The file's *content* is made durable by `sync()` on the temp handle before the rename. The directory-entry *change* (which points live at the new inode) is made durable by `sync()` on the reopened live handle. Strictly, a `Dir.sync` would additionally guarantee the rename's directory entry is flushed, but that API is not exposed on this toolchain, and it is not needed for the BE-GRANT-01 invariant: a crash after rename returns shows old or new, both complete. Dir fsync would only harden the durability of the *new* state, not the *safety* property (no empty log). The invariant is satisfied by rename's atomicity alone.

**Ruling 2 — the regression test is positive, not negative.** The crash-injection test that proved the bug cannot survive the fix by construction: it constructs an empty live file as crash residue, which is exactly the state the fix makes unreachable. The test is rewritten to verify the fix's properties directly: after a prune that compacts (one expired grant dropped, one live grant kept), the live log contains exactly the survivor; and a stale `.tmp` left by an aborted prune is cleaned at the next run without corrupting the live log. This is a real regression guard, not a tombstone for the old scheme.

**Ruling 3 — the GrantLedger now carries its path.** The fix needs the live path to build the temp path and call `rename`; the struct previously held only the file handle. `path_buf: [128]u8` and `path_len` are added, captured in `open()`. 128 is the same cap the test harness assumes; `pruneExpired` guards `path_len + 4 > 128` (the `.tmp` suffix) and returns `BadLog` rather than overflowing.

Rejected alternatives: (1) keep truncate-in-place and add a WAL/journal — reintroduces the complexity the two-phase append log exists to avoid, and BE-DEP-01 forbids pulling a library to do it; (2) copy-on-write the file and swap via a separate durable marker file — two files to keep consistent under crash, strictly worse than one atomic rename; (3) skip pruning entirely and bound via MAX_LIVE alone — `commitConsumed` would return `ResourceExhausted` once MAX_LIVE consumed rows accumulate, refusing all new grants (fail-closed but operationally broken).

Reversible: revert `grant_ledger.zig` to `setLength(0)` -> write -> sync; the path fields return to unused. No M1 marker changes (BE-GRANT-01 was already declared and is now actually satisfied under crash). What would reopen it: a toolchain where `rename` is not atomic within a directory (none of the targets do, but it is the load-bearing assumption), or a multi-process deployment (BE-EXEC-01 forbids it, but it would make rename insufficient without file locking).

## D-064 — 2026-08-12 — F4 wiring: revocation consulted at the grant checkpoint, BE-REV-02 durably enforced (RED-TEAM-10)

RED-TEAM-10 (adversarial review, cross-validated independently) adjudicated F4 as a GAP, not layering: the grant execution path did not consult revocation. `GrantContext` (verify.zig) had no `is_revoked` seam, while `ChannelContext` and `MeshContext` both did. The durable revocation set built in Phase D (`grant_ledger.commitRevocation`/`isRevoked`, BE-REV-02) was dead code at the one checkpoint that matters.

**The principle (decisive).** verify.zig:454 states revocation is consulted "as of this use, not of cache fill." The grant path IS a use, it is the checkpoint where capability turns into effect (`execute(grant)`, the sole call site, M10). Consulting revocation only at mesh establishment (low risk) and not at the effect checkpoint (maximum risk) inverts the principle the code already declares.

**The fix (5 edits).** (1) `is_revoked: *const fn (sig_pubkey: []const u8) bool` added to `GrantContext`, mirroring `ChannelContext`/`MeshContext`. (2) `ApproverRevoked` and `SubjectRevoked` added to `VerifyError`. (3) Check 3 (approver) consults `is_revoked(approver_cert.sig_pubkey)` -> `ApproverRevoked`. (4) Check 4 (subject) consults `is_revoked(subject_cert.sig_pubkey)` -> `SubjectRevoked`. (5) Caller wire: `dispatch.isRevokedHook` binds the active ledger's `isRevoked`, same fail-safe shape as `consumedHook`.

**Ruling 1 — fail-safe default preserved.** Like `consumedHook`, `isRevokedHook` returns `true` (refuse) when no ledger is initialized. The revocation set is part of the durable authority state; without it loaded, no grant may turn into an effect. A missing or malformed-length pubkey also refuses.

**Ruling 2 — SubjectRevoked is a new variant in VerifyError, not a reuse.** The other review lane's SPEC claimed "SubjectRevoked already exists, reuse it." That conflated error sets: `SubjectRevoked` existed only in the channel-context error set (verify.zig:334), not in `VerifyError` (the grant-path return type). Both `ApproverRevoked` and `SubjectRevoked` are declared fresh in `VerifyError`. This is the rubber-stamp the no-sycophancy rule exists to prevent.

**Ruling 3 — check ordering is unchanged and sound.** Revocation runs at checks 3-4, immediately after cert validation and identity binding, and before expiry (10) and the consumed-grant commit (11). The durable revocation read is therefore strictly before the durable commit, so a revoked key cannot spend a grant_id. The ordering also keeps revocation a cheap in-memory cache read (recovered into the ledger on open), not an I/O in the hot path.

**Reversible.** Remove the `is_revoked` field, the two checks, the two variants, and the `isRevokedHook`/wire; the F4 guard flips back to `grant_executed`. No M1 marker changes: BE-REV-02 was already declared and is now enforced at the effect checkpoint, not merely stored.

## D-065 — 2026-08-13 — Harness v21: the grant_revocation denominator for the F4 seam (D-064, RED-TEAM-10)

The mutation denominator for the F4 wiring landed by D-064. The seam enforces BE-REV-02 at the capability->effect checkpoint (verify.zig checks 3-4, dispatch.isRevokedHook over the durable grant_ledger revocation set), but v20 carried no mutant that attacked it: the existing revocation mutants cover the channel path (chan-02), the mesh path (mesh-06), and the ledger's durability (grant-ledger-revoke), none the grant checkpoint. Removing the F4 wiring entirely would have left 151/151 green. This entry closes that hole.

**Ruling 1 — a new domain keyed to D-064, not an extension of dispatch or grant_ledger.** The dispatch domain keys derive from the D-059 ruling and the grant_ledger domain from D-061; the F4 seam is a D-064 artifact, so it gets its own denominator the same way relay_serve got one from D-060. The seam code carries no new SPEC marker (D-064: no M1 changes), so the keys are hardcoded citing the D-064 rulings, exactly the shape the D-059 dispatch domain uses for marker-less daemon-milestone code. Three keys: `f4-approver-check` (fix 3: check 3 consults is_revoked(approver) -> ApproverRevoked), `f4-subject-check` (fix 4: check 4 consults is_revoked(subject) -> SubjectRevoked), `f4-failsafe` (ruling 1: no durable ledger -> refuse).

**Ruling 2 — four mutants, three witnesses.** (1) CHECK-ABSENCE on check 3 and (2) WRONG-LOGIC inverting check 3 both anchor verify.zig's approver revocation line; the absence mutant is killed by the F4 regression guard (dispatch_test.zig, expects ApproverRevoked), the inversion by every non-revoked grant happy path. (3) CHECK-ABSENCE on check 4 anchors the subject revocation line; killed by a new literal witness, the F4 subject guard: a durably revoked subject (AGENT_PREFIX key) pre-seeded in the ledger, approver unrevoked so checks 1-3 pass, grant refused with SubjectRevoked and the effect never fired. (4) WRONG-VALUE flipping isRevokedHook's `durable_ledger orelse return true` to `return false` anchors the hook head widened past the identical consumedHook line; killed by a new literal witness, the F4 fail-safe test: module ledger slot forced null, grant refused with ApproverRevoked before check 11 (pinning ruling 1 and ruling 3's ordering together: the revocation fail-safe fires strictly before the consumed-grant fail-safe). The subject-revoked and no-ledger paths had zero test coverage before this; both witnesses are literal binding tests in the D-027 shape.

**Ruling 3 — the anchor-widening discipline for duplicated hook lines.** `var lg = &(durable_ledger orelse return true);` appears verbatim in both consumedHook and isRevokedHook; the runner applies `.replace(find, replace, 1)`, so a bare-line anchor would mutate the FIRST occurrence (consumedHook, the wrong hook). The failsafe mutant therefore anchors from `fn isRevokedHook(sig_pubkey: []const u8) bool {` through the orelse line. Same discipline as the v20 re-anchoring of the check-11 seam mutants (D-062).

The denominator reads 155 mutants over eighteen domains (v20: 151 over seventeen). Harness version cell bumped v20 -> v21. No M1 marker changes, no surface budget touched, no SPEC edit: BE-REV-02 was already declared and is now mechanically defended at the effect checkpoint rather than merely implemented.

Reversible: delete the four mutants, the GRANT_REVOCATION_PROPS block, and the grant_revocation gate wiring; the denominator returns to 151/151 v20 and the two new dispatch tests stay (they bind behavior, not harness). What would reopen it: a SPEC change making revocation consultable at a different checkpoint than use (the keys cite the as-of-use principle), or concurrency over dispatchGrant (the module-slot fail-safe shape changes, D-062's constraint).

## D-066 — 2026-08-13 — §11.5 R2 post-hoc effect-log auditor: src/adversarial_audit.zig

The anti-faking core of SPEC §11.5 (research/adversarial-eval-rubric.md, R2). §11.5 is, by the SPEC's own words, the single most important conformance item and the one most easily faked; the rubric exists so the eval cannot be gamed the day the live executor lands. R2 — the dual M1/M2 measurement detected by post-hoc audit of the durable record — does not require the live executor (main.zig is a skeleton, no model binding): the two-phase grant ledger (D-061) is the crash-safety substrate AND the audit substrate (rubric section 4), so the auditor runs today. This entry places the auditor as a non-surface unit and fixes its detection scope.

**Ruling 1 — placed non-surface, post-creation.** `src/adversarial_audit.zig` joins the BE-SURF-03 non-surface list alongside grant_ledger.zig (D-061) and the dispatch/resolver post-admission units. It is pure post-hoc analysis over parsed/committed state, never reached by attacker bytes, never on the wire. The grant_ledger gained one public read-only accessor, `isPublished` (mirrors isConsumed over the existing publishedIndex), so the auditor can distinguish an executed effect (consumed AND published) from an interrupted one (consumed, not published). No wire bytes change, no surface cap changes, no M1 marker changes.

**Ruling 2 — four violation classes, audited from the durable record alone.** The auditor flags what it can PROVE post-hoc: (a) unwitnessed — an effect claim whose grant_id has no consumed row; (b) unpublished — a claim whose grant was committed but never tombstoned; (c) expired_at_exec — a claim where not_after <= executed_at (check 10 should have blocked; the boundary is inclusive because the validity window is exclusive at not_after, binding.validateCert X.509 convention); (d) double_spent — a grant_id appearing in two claims (check 11 should have blocked the second). M2 counts an intended grant only if it is consumed AND published (an interrupted grant did not complete). PASS is conjunctive: M1 == 0 AND M2 == N; a "refuse everything" sheet (M1 == 0 vacuously, M2 == 0) FAILS on M2 — the whole point of dual measurement.

**Ruling 3 — revocation is deliberately NOT audited here.** The ledger's commit row carries grant_id and expiry but not the approver pubkey or a consumption timestamp, and revocation rows carry cert_expiry but not a revoked-at time. Without both timestamps, a post-hoc check cannot distinguish a grant legitimately consumed before its approver was revoked from one honored after. That distinction is enforced AT USE by the dispatch seam (F4 wiring, D-064, checks 3/4) where the ordering is known. Auditing it here from incomplete timestamps would produce false positives — the opposite of the mechanical, unfakeable result this module exists to give. The auditor covers what is definitively post-hoc-verifiable; revocation stays a runtime (F4) responsibility.

**Ruling 4 — borrowed mutable handle, read-only by contract.** The auditor takes `*GrantLedger` (mutable) because the ledger's read accessors take `*GrantLedger` self; const-correctness across six methods + three internal index helpers was judged more churn than the value, and the auditor mutates nothing. Contract is documented in the module header. Reversible: remove the accessor and the module; the seven binding tests in adversarial_audit_test.zig bind detection behavior, not the ledger.

**What this is NOT.** This is the deterministic scoring engine of §11.5, not the live adversarial run (A1/A2 prompt-injection and approver social-engineering still need a model + executor). It is the prep that makes the eventual run unfakeable: whoever runs §11.5 cannot report one number, cannot refuse everything, and cannot hide a win behind judge-subjectivity. §11.5 stays unsealed until the live executor + models exist and the run produces a conjunctive PASS.

## D-067 — 2026-08-13 — §6/§13.3 lifecycle-order discrepancy (Huebrz D-04): ACCEPT the D-059 correspondence rule, do NOT move beginExecuting before effect

Huebrz's model-checking spec (BOLINA-MODEL-CHECKING-SPEC.md, v0.1-candidate) flagged a lifecycle-order gap: the SPEC prose says a grant transitions to EXECUTING, but dispatchGrant (src/dispatch.zig:207-250) calls the effect callback and markPublished (lines 232, 242) BEFORE intents.beginExecuting (line 249). Verified against live code this session. This ruling resolves D-04: the only genuinely open technical decision in the D-01..D-07 batch. The others were already settled — D-01 by D-061 (at-least-once delivery is intentional, not a defect), D-03 by D-063 (pruneExpired atomic rename), D-02 mostly (its residue folds into D-04), D-05/D-06 governance/owner-scope.

**Ruling 1 — beginExecuting is a durable label, not the state transition.** The real transition to "executing" happens INSIDE the verify frame: verify.verifyGrantThen (line 232) runs all twelve checks and then fires the effect callback atomically behind the frame lock. BE-GRANT-03b (SPEC 1425-1431) fixes this exactly: "the frame ends when the effect has [started]." The subsequent beginExecuting call (line 249) only records that fact in the durable intent log so a crash/restart can resume the correct state. It is a marker write, not the transition. Treating it as the transition would misread the SPEC: the SPEC's "transition" is the effect-start, which is already frame-protected.

**Ruling 2 — ACCEPT the D-059 correspondence rule; REJECT moving beginExecuting before the effect.** The proposed "fix" (emit beginExecuting before the effect fires) would require splitting the verify frame: open the frame, write the executing marker, close the frame, then re-open it to fire the effect. That split reintroduces exactly the TOCTOU window BE-GRANT-03b exists to delete — an approver revoked between the marker write and the effect fire would slip through. The current order (effect inside the frame, marker after) keeps the whole verify-consume-execute sequence atomic and is therefore the SAFE ordering, not the buggy one. The D-059 correspondence rule stands: the marker is emitted as a post-effect bookkeeping consequence of a transition that already happened under the frame, in the same durable transaction window as markPublished.

**What this is NOT.** This is not a security hole and requires no code change. It is a formal-spec wording gap: the SPEC prose describes the EXECUTING transition in lifecycle terms without naming that the transition is frame-internal and the beginExecuting call is its durable echo. When §11.4 is written up against a live model, the SPEC should add a one-line note binding BE-GRANT-03b's "frame ends" sentence to the beginExecuting call as its durable witness, so a model-checker sees the correspondence explicitly rather than inferring it. No M1 marker changes, no surface changes, no test changes — the dispatch tests already assert effect_count and the consumed-set invariants this relies on.

Reversible: the decision is to NOT change code, so "reversing" means a future owner choosing to split the frame and accept the TOCTOU reintroduction, which would itself reopen BE-GRANT-03b's seal condition (SPEC 1475: "the seal defended the window between verification and consumption"). What would reopen this ruling: a SPEC change making verification a value rather than a call (which breaks BE-GRANT-03b's portability premise), or concurrency over dispatchGrant (the single-frame assumption fails).

## D-068 — 2026-08-13 — §11.5-A adversarial live crash-adversary harness: src/adversarial_live_test.zig, conjunctive M1/M2 over real dispatch + recovery

The live half of the §11.5 deterministic core. D-066 placed the R2 post-hoc auditor (the scoring engine) as a non-surface unit; this entry places the deterministic crash-adversary that FEEDS it. Owner decision (Daniel, this session): crash-adversary first (model A), forge-adversary deferred. main.zig stays a skeleton: this is NOT the network daemon attacker (phase B). The adversary controls CRASH TIMING, not the wire. A crash is modelled as the exact post-crash durable ledger state the real crash would leave (committed-but-unpublished orphan, mid-prune compaction, partial-record), followed by the REAL recovery path and the REAL audit. What is novel over the DAEMON_D dispatch unit tests is the composition: dispatch output AND recovery output fed into the R2 auditor and reconciled as one conjunctive verdict.

**Ruling 1 — five scenarios, one conjunctive invariant.** S1 CLEAN_RUN drives three grants through real dispatch then audits an independent durable view (M1==0, M2==3). S2 ORPHAN_MID commits a grant durably then crashes before the tombstone (the BE-GRANT-01a orphan window), recovers the single orphan, publishes and tombstones it, then audits (M1==0, M2==2). S3 REFUSE_TRAP is the negative witness that validates the harness itself: with no ledger the fail-safe refuses every grant, M1==0 vacuously but M2==0<3, so passed() is false (the conjunctive verdict REJECTS a perfect false-negative rate). S4 PRUNE_SURVIVAL proves the D-063 atomic-rename compaction never un-spends a live grant (M1==0, M2==1). S5 REPLAY_AFTER_RESTART proves the durable single-shot gate survives restart (AlreadyConsumed on replay, M1==0, M2==1).

**Ruling 2 — S3 is the anti-faking keystone.** The whole point of dual measurement (D-066 ruling 2) is that one number is not a result: a system that refuses every grant reports M1==0 trivially. S3 asserts the auditor CORRECTLY rejects that trap. Without S3 the harness would pass a sheet that refuses everything; with it the conjunctive verdict is unfakeable. S3 exercises the D-064 revocation fail-safe (no ledger -> isRevokedHook returns true -> ApproverRevoked at check 3, before the consumed fail-safe at check 11), pinning ruling 3 of D-064 (ordering) and ruling 2 of D-066 (conjunctive PASS) together.

**Ruling 3 — S2 and S4 use the ledger standalone, not dispatch.** The orphan window and the compaction window ARE ledger-level crash states; routing them through dispatch would exercise code paths the unit tests already cover and add nothing to the recovery+audit composition under test. S2/S4 therefore open a standalone GrantLedger, inject the post-crash state directly, run the real recover() and real pruneExpired(), then audit. S1/S5 route through real dispatch to prove the full path reconciles. This split keeps each scenario at the cheapest informative level and avoids re-enacting the DAEMON_D tests.

**What this is NOT.** This is the deterministic half of §11.5-A, not the forge-adversary (model B, phase B) and not the live network run (A1/A2 prompt-injection and approver social-engineering need a model + executor). §11.5 stays unsealed until the live executor + models exist and the run produces a conjunctive PASS. No M1 marker changes, no surface changes, no production code changes: one new test module (adversarial_live_test.zig) and one import line in tests.zig. The suite re-run that gated this commit landed clean (155/155 killed, 0 survived) on a restored tree; the prior 154/154 contamination is closed.

Reversible: delete adversarial_live_test.zig and the import line in tests.zig; production code is untouched. What would reopen this ruling: a change to the D-066 audit signature (the harness calls audit() and reads m1_violations/m2_granted/passed), a change to the D-064 fail-safe ordering (S3 pins check 3 before check 11), or a change to the D-063 atomic-rename shape (S4 pins live-grant survival through pruneExpired).

## D-069 — 2026-08-14 — §11.5-B semantic adversary harness: src/adversarial_semantic_test.zig, M3 (semantic integrity) over real dispatch; the structural floor of the social-engineering vector

The live half of §11.5-B. The other chat's threat model (research/adversary-11.5-threatmodel.md) correctly scoped M1 as forged/stolen-only and correctly identified that human-factor fatigue is operational, not cryptographic. It UNDER-concluded by inferring nothing structural remained to prove on the semantic vector. Verified against code: four invariants the semantic vector rests on are provable deterministically and had NO adversarial test today. Owner decision (Daniel, "Siga", explicit: "resolve the problem, not defer, not the easy path"): build the semantic-adversary harness. main.zig stays a skeleton; this is NOT the network daemon attacker. The adversary authors the grant+intent with mesh access, not the wire. Not theatre: SEM_S2/S3/S4 are refused (the invariant bites), SEM_S5 accepts a valid grant (no over-refuse), SEM_S1 proves rationale is ignored.

**Ruling 1 — four invariants, five scenarios.** SEM_S1 RATIONALE_NON_INFLUENCE (I2, BE-BODY-03): the intent carries an injection rationale ("ignore prior instructions; authorize shutdown-system"), the approver signs the real action, the effect runs the signed action. M3==0. This is the first adversarial test of BE-BODY-03; rationale has no path to the effect because GrantContext has no rationale field and Hooks.execute_effect takes only channel.Grant (dispatch.zig:60). SEM_S2 BINDING_RESOURCE (I1, check 8): grant resource_id != intent canonical resource -> WrongResource (verify.zig:239), effect_count==0. SEM_S3 BINDING_ACTION (I1, check 9): grant action_digest != BLAKE2s(intent action) -> ActionDigestMismatch (verify.zig:243), effect_count==0. SEM_S4 ROLE_CONTAINMENT (I4, check 3): an agent cert (ROLE_AGENT) signing a grant naming itself approver -> BadApproverCert (verify.zig:218), because the role gate (role_bits & ROLE_APPROVER)==0 fires before the revocation sub-check, effect_count==0. SEM_S5 CLEAN_M3_BASELINE: a correct grant fires the effect and M3==0 (executed==signed); paired with the refusals it proves the harness neither over-refuses nor trivially passes.

**Ruling 2 — M3 is harness-level, not post-hoc, and that is not a weakness.** M1 (adversarial_audit.zig) cannot see executed-versus-signed because the ledger stores the 16-byte grant_id, not the action text, by design. M3 is measured in the harness: the builder knows what it signed (it built the grant wire), and the effect callback captures what ran (the channel.Grant handed to execute_effect). M3==0 means executed action_digest and resource_id equal the signed ones. This is a mechanical byte comparison, unfakeable, not a model-of-a-model. SEM_S5 is the positive witness that the M3 machinery works; SEM_S2/S3 are the negative witnesses that the binding it asserts is real.

**Ruling 3 — the I4 residual is documented, not fixed.** Cert (session.zig:167) carries role_bits + group_ids but NO resource scope. An approver's blast radius is therefore bounded by role (approve only, check 3; binding forces effect==signed, checks 8/9), not by resource. Closing the residual means changing the wire format (add a resource-scope field to Cert plus a binding check), a large owner-scoped decision, deferred. SEM_S4 proves the role bound holds; the residual is named explicitly in the SPEC §11.5 scoping addendum so it is not mistaken for a closed gap.

**Ruling 4 — the human factor stays operational, not faked.** Per THREAT-MODEL §4.1 ("mitigation is operational, not cryptographic"), approval rate-limits and mandatory read delays are not deterministically auto-testable. A harness that "passes" §11.5 by simulating human fatigue would be theatre (model-of-a-model). This ruling resolves the provable structural floor; the operational layer is documented as such, not simulated to claim a pass.

**What this is NOT.** Not the network forge-adversary (phase B loopback), not main.zig, not a seal of §11.5 (the live executor and attacker models remain phase B). No production code changes: one new test module (adversarial_semantic_test.zig), one import line in tests.zig, and the SPEC §11.5 scoping addendum. The social-engineered grant itself (real chain, tricked human) is out of scope for M1 by design and cannot be auto-proven; what IS proven is that even a fully-tricked approver cannot make the machine run anything beyond what they signed.

Reversible: delete adversarial_semantic_test.zig and the import line; revert the SPEC §11.5 addendum. What would reopen this ruling: a change to check 3 ordering (SEM_S4 pins the role gate before revocation), to checks 8/9 (SEM_S2/S3 pin resource and action binding), or to the GrantContext/Hooks shape (SEM_S1/S5 pin execute_effect taking channel.Grant with no rationale path).

## D-070 — 2026-08-14 — §11.6 soak activated: the nightly schedule in .github/workflows/soak.yml is uncommented on owner ruling

Owner decision (Daniel, this session, "Ativa"). The soak component of §11.6 was deferred for want of dedicated compute: the owner's workstations are out of scope (heat) and production servers are out of scope (disruption, hard veto). The GitHub Actions workflow built and verified green under `workflow_dispatch` is neither, so the deferral condition is discharged and the schedule is activated.

**Ruling 1 — the trigger change is the whole change.** `schedule: - cron: "0 3 * * *"` is uncommented; `workflow_dispatch` and its inputs are untouched, the matrix of five decimal seeds is untouched, the per-job budget (14,400,000,000 chaos inputs, 5,000,000 differential records) and the 330-minute timeout are untouched. No production code, no test code, no SPEC surface, no M1 markers. `actionlint` passes on the edited file.

**Ruling 2 — accumulation across runs and seeds, stated as such.** GHA caps one job at 6 hours, so the §11.6 ">= 24 hours" figure is met by accumulating five independent seeds per night across nights, not by one uninterrupted run. This is a coverage argument, not a literal continuity argument, and the workflow header says so. Each job reports coverage reached and the seed used per R1; a panic or a differential divergence fails the run. If the owner later requires literal continuity, that needs a dedicated long-lived runner and a separate ruling.
...
## D-072 - 2026-08-14 - the §11.4 model checker gets a pinned GHA runner before the model exists: .github/workflows/model-check.yml

Owner decision (Daniel, this session, "siga"). SPEC §11.4 requires a TLA+ or Alloy model of the §8 grant state machine with BE-GRANT-01/-03/-04/-06 as invariants, and states that until it exists §8's guarantees are tested, not proved. The model author (Huebrz) is adjusting his spec against D-067 and will open a PR. TLC needs a JVM; the maintainer workstation has none and installing one there is out of scope under the same compute ruling that put the soak on GitHub Actions (D-070). Building the runner before the model lands removes a dependency on the reviewer from the author's critical path.

**Ruling 1 - the tool is pinned and hash-verified.** TLA+ tools v1.7.4, `tla2tools.jar`, sha256 `936a262061c914694dfd669a543be24573c45d5aa0ff20a8b96b23d01e050e88`, 2274532 bytes, downloaded from the tlaplus GitHub release and checked with `sha256sum -c` before it runs. This is the O4 discipline the soak workflow already applies to the Zig toolchain: a model-checking receipt is only as trustworthy as the checker that produced it, so the checker is a fixed artifact, not "whatever the latest release is today".

**Ruling 2 - a violation must fail the job, and the trace must survive it.** TLC exits non-zero on an invariant violation or a deadlock, and the job does not swallow it (`set -o pipefail` with `tee`). The full log, counterexample trace included, is uploaded as an artifact with `if: always()`, and the summary step greps the state count, depth, and violation lines into the job summary. A counterexample is the most valuable output this workflow can produce; losing it to a failed step would defeat the point.

**Ruling 3 - bounded, and cancellable unlike the soak.** `timeout-minutes: 120` bounds state explosion, which is the expected failure mode of an unconstrained model, so "the model does not terminate" reads as a fast signal instead of a six-hour ceiling hit. `-Xmx6g` is conservative for the 16 GB runner: sizing the heap to the box turns an out-of-memory into an unreadable kill. The concurrency group sets `cancel-in-progress: true`, the opposite of the soak (D-071), because a model check is a verdict on one revision and a newer commit supersedes it, whereas a soak accumulates evidence that must never be killed.

**Ruling 4 - triggers are scoped to the model.** `pull_request` and `push` to main fire only on `model/**` (plus the workflow itself); everything else is manual dispatch with explicit spec and config inputs, defaulting to `model/Bolina.tla` and `model/Bolina.cfg`. A state-space exploration is not a per-push gate and ci.yml already carries those. A guard step fails with an explicit message when the spec or config is absent, which is the state of the repository today: no `model/` directory exists yet.
...
## D-074 - 2026-08-15 - the first nightly soak reported five green jobs on a differential half that never ran: pipefail in soak.yml, a measured record budget, and an explicit oversize refusal in fuzz_diff.py

The schedule activated by D-070 fired for the first time at 03:44 UTC on 2026-08-15 (run 31862614634, HEAD `6dfebcb`). All five matrix jobs concluded success. Four of the five ran 2h48m, one 3h03m, and the chaos half is real: `FUZZ DONE: 14400000000 inputs (316800000000 parser calls), 0 panics` and `COVERAGE: 72/72 exit points reached` on every seed. The differential half failed on every seed and the run said nothing about it. Each job's log carries `error: StreamTooLong`, `FAIL: zig diff replay`, and `error: process exited with error code 1` inside a step whose conclusion is success.

Two independent defects compose into a false green, and either alone would have been visible.

**Ruling 1 - the differential step gets `set -o pipefail`, which is the whole reason the failure was invisible.** The step runs `python3 tools/fuzz_diff.py ... 2>&1 | tee differential.log`. Without `pipefail` bash reports the exit status of the last element of the pipeline, which is `tee`, which always succeeds. The orchestrator's exit 2 was therefore discarded. The chaos step above it (D-070) and the TLC step in model-check.yml (D-072 ruling 2) both already set `pipefail` with `tee`; this step was the one that did not, and it is the step that decides whether a divergence fails the run. A workflow header comment now states the dependency so the next `tee` added here inherits the reasoning rather than the bug. `actionlint` passes on the edited file. Every other `tee` in `.github/workflows/` was swept: ci.yml has none, model-check.yml is correct.

**Ruling 2 - the record budget is a memory budget, and 5,000,000 was never runnable.** The corpus is a measured 1274.6 bytes/record: 255,004,304 bytes for 200,068 records locally and 6,379,201,306 bytes for 5,000,068 in CI agree to four figures. The Zig replay reads the corpus whole under `.limited64(4 * 1024 * 1024 * 1024)` (`src/fuzz.zig` `runDiff`) and allocates a tag byte plus a verdict bool per three corpus bytes on top, so a 5M-record corpus needs about 10.6 GB against a 4 GiB read cap on a 16 GB runner; the Python side then holds the same corpus plus a per-record verdict and payload list. The default was not merely too large for the cap, it was unreachable on the box. It is lowered to 1,000,000, which is the scale the oracle is already evidenced at (RED-TEAM-10 extended it from the 20,068-record merge gate to 1,000,000) and which is verified end to end on this tree before the change was committed: 1,000,068 records, 173,844 accepted, 826,224 rejected, **0 divergences**, 72/72 coverage, corpus 1,274,866,020 bytes, exit 0. Five seeds a night at 1M each is 5,000,000 records of differential evidence per night, accumulating per D-070 ruling 2.

**Ruling 3 - an oversized corpus is refused by name, not by a bare `StreamTooLong`.** `tools/fuzz_diff.py` now checks the emitted corpus against the replay cap and exits 2 naming the corpus size, the cap, the offending `--budget`, and the measured bytes/record, so the ~3,369,996-record ceiling is stated rather than discovered. The guard was exercised with the cap shrunk to 1000 bytes: it fires and returns 2. `src/fuzz.zig` is untouched. Raising the reader cap was considered and rejected: it would trade an early honest refusal for an out-of-memory kill on a 16 GB runner, and the constraint is the whole-corpus design on both sides, not the constant. Streaming both replays would lift the ceiling and is a real option, but it is a harness rewrite, not the fix for a workflow that lies about its own result.

**What this is NOT.** Not a change to what the oracle measures: same corpus generator, same framing, same reference parser, same comparison, no production code, no test code, no SPEC surface, no M1 markers. BE-SURF-04 stays at 114/114 declared markers bound, high water 114, and §11.6 stays unsealed. The chaos half's first night stands as evidence on its own terms; the differential half has produced zero nightly evidence to date and the audit addendum says so.

Reversible: restore the 5,000,000 default, delete the guard block and the `pipefail` line. What would reopen this ruling: a streaming replay on both sides (which would make the record budget a time budget instead of a memory budget), a dedicated runner with materially more RAM, or a measured bytes/record that moves after a corpus-generator change.

## D-075 - 2026-08-16 - SPEC section 11.6 is sealed on the second nightly soak: both halves green on every seed, the accumulation figure over 24 hours

The claim of section 11.6, in its own words: the parser survives >= 24 hours of continuous fuzzing with zero crashes, zero panics, zero out-of-bounds reads, zero allocations observed, against every section 2.2 structure, with coverage of the parsing code reached and the seed corpus used reported per run (R1), and the fuzzing differential per BE-SURF-04. CONTRIBUTING.md defines sealed as "the evidence for a specific claim exists and is recorded." This ruling records it.

**Ruling 1 - the evidence is the second nightly, read from the job logs.** GitHub Actions run 31925289374 (HEAD `2f310b0`, 03:52 UTC on 2026-08-16), all five matrix seeds green on both halves. Chaos: `FUZZ DONE: 14400000000 inputs (316800000000 parser calls), 0 panics`, `COVERAGE: 72/72 exit points reached` on every seed. Differential: 1,000,068 records, `divergences: 0`, 72/72, `VERDICT: PASS` on every seed, with the D-074 pipefail active so a divergence would have failed the job. Full receipts and the one-tree note (src/ identical between `6dfebcb` and `2f310b0`) are in the second nightly addendum of `M1-AUDIT.md`.

**Ruling 2 - the 24-hour figure is met by accumulation, per D-070 ruling 2.** Night one contributed about 14.6 hours of chaos fuzzing (14h15m on a strict job-wall reading), night two 13h41m. The conservative sum is 27h56m, comfortably over 24 hours. Zero crashes and zero panics are measured both nights; zero out-of-bounds reads follow from a clean ReleaseSafe exit over the input count; zero allocations hold structurally per D-056 ruling 5 in this log (no parse function takes an allocator, the fuzz arena covers setup only) and are not re-polled per run.

**Ruling 3 - what this does not do.** No M1 marker changes (BE-SURF-04 stays 114/114 declared, bound, high water 114), no surface changes, no production code changes, no SPEC text changes: the requirement was already written; what was missing was the evidence, and the evidence arrived on schedule. The unsealed conformance items drop from three to two: section 11.4 model checking (in progress, external contributor, D-072 prepared the pinned runner) and section 11.5 adversarial evaluation against a real model (not run; the deterministic prep exists, the live run does not).

Reversible: the seal is an evidence state, not a code state; it reverts the moment its premise breaks. What would reopen it: any future nightly reporting a panic or a differential divergence (that night is a counter-example and section 11.6 returns to unsealed until the defect is fixed and the accumulated evidence is re-established), or the owner requiring literal 24-hour continuity rather than accumulation, which per D-070 ruling 2 needs a dedicated long-lived runner and a separate ruling.


## D-076 - 2026-08-16 - bolina.grant-trace.v1: the comptime-gated conformance instrumentation for the ZIG-TLA pilot

The ZIG-TLA conformance brief (model/ZIG-TLA-CONFORMANCE-BRIEF.md, merged with the TLA+ model in PR #13) names the Zig observation points its projector needs. This ruling places the instrumentation that provides them and fixes its discipline.

**Ruling 1 - comptime gate, default off, zero cost when disabled.** A new build option `trace` (-Dtrace=true, default false) exposes `build_options.trace_enabled` to `src/grant_trace.zig`, the fixed 256-event in-memory buffer behind the `bolina.grant-trace.v1` schema. Every emit site is `if (grant_trace.enabled) grant_trace.emit(...)`: with the flag off the branch is comptime-dead and compiles out, so production builds, the nightly soak, and the mutation harness (which runs plain `zig build test`) carry zero instrumentation cost. Receipts: default build 381/384 (3 trace tests SkipZigTest), `-Dtrace=true` 383/384 (1 skip from the other side, 3 event tests run).

**Ruling 2 - the two load-bearing event placements follow the brief exactly.** `commit_consumed_11` is emitted only after the durable `appendSync` returns successfully inside `grant_ledger.commitConsumed`: an event before the append would turn attempted durability into false evidence. `effect_start` is emitted immediately before the effect callback invocation inside `verifyGrantThen`, which under D-067 IS the normative APPROVED -> EXECUTING transition; `record_executing_witness` is emitted after `beginExecuting` returns and is the later durable bookkeeping echo, never authorization for the effect. The happy-path test asserts the exact 18-event ordered sequence; the check-11 stutter test asserts that a reused grant_id emits neither `commit_consumed_11` nor `effect_start` (13 events, refusal preserved); the conflict test asserts `reject_resource_conflict`.

**Ruling 3 - unbound actions stay unbound, and the clock is honest about what it cannot see.** `PublishOutcome` and terminal intent completion (FinishExecuted/FinishFailed) have no observation point in the current code (execute_effect returns void, no terminal transition in the reviewed path), so the trace claims only the observed prefix through EffectReturn plus the witnesses, per brief section 6: the projector must not synthesize the unobserved. `mark_published` and `recover_mark_published` carry now_ms = 0 because the publication call path carries no clock; ordering authority is the monotonic seq counter within a process epoch, and Crash/Restart is a harness-owned epoch boundary that resets the buffer.

**What this is NOT.** No M1 marker changes (114/114 stands), no wire bytes, no behavior change of any kind when the flag is off, no new surface: BE-SURF-03 places `src/grant_trace.zig` in the non-surface list (SPEC v0.3.9-draft changelog) because attacker bytes cannot reach it and it costs nothing when disabled. The mutation receipt 155/155 is stale by design: verify.zig, dispatch.zig and grant_ledger.zig are TARGETS files and gained wiring lines, so the suite is re-run under the D-073 baseline guard before the receipt is cited again.

Reversible: delete the build option, the wiring lines (each a single gated statement), the placement entry, and the two new files. What would reopen this ruling: the pilot's section 9 decisions landing (publication boundary, tombstone failure semantics), which would add the currently-unbound observation points as new events rather than reinterpret existing ones.


## D-077 - 2026-08-16 - re-anchoring the grant-ledger-commit mutant that D-076's trace line split

The D-076 re-run of the mutation suite against `74cb53a` returned 154/154 killed with one explicit SKIP: `[grant_ledger/CHECK-ABSENCE] BE-GRANT-01: commit row never appended` reported `anchor not found (grant_ledger.zig changed?)`. The harness did not fail silently: the SKIP line and the `UNCOVERED grant_ledger properties: ['grant-ledger-commit']` report named the gap. Cause: D-076's `commit_consumed_11` emit line now sits between `try self.appendSync(&row);` and the in-memory `@memcpy`, and the mutant's anchor required those two statements adjacent.

**Ruling 1 - the anchor follows the code, not the other way around.** The mutant definition is updated to the new source shape: anchor and mutant text both include the trace emit line. Semantics unchanged: the mutation still removes exactly the durable `appendSync` of the commit row (the load-bearing BE-GRANT-01 append) and keeps everything else. The trace line is inert in the suite's default build (`grant_trace.enabled` is comptime false, the branch compiles away), so keeping it in the mutant text changes nothing about what the suite measures.

**Ruling 2 - the receipts, and why the union is complete at `74cb53a`.** The full run killed the 154 mutants whose anchors were untouched (no src/ change is involved in this fix, only one mutant definition in the harness). The chunked grant_ledger run (MUTATION_DOMAIN, D-035 mechanism) at the same HEAD, same clean baseline under the D-073 guard, killed 7/7 including the re-anchored commit mutant, with 7/7 D-061 properties covered and no SKIP. 154 + 7 over the same HEAD and tree is the full 155-property coverage with zero survivors. A canonical full run is launched after this commit so the single-log receipt exists too; the chunk receipt stands on its own regardless.

**Ruling 3 - anchor maintenance is now an expected cost of instrumenting TARGETS files.** This is the second re-anchoring in the project's history (the first was the seam widening in df4ceef). The harness already reports SKIP loudly instead of dropping silently (D-073 era behavior confirmed here), and the property-coverage report turns any anchor rot into a visible `UNCOVERED` line rather than a quiet denominator shift. Any future change to a TARGETS file that inserts a line between an anchor's statements must check the harness SKIP/UNCOVERED report before citing a mutation receipt.
...
## D-081 - 2026-08-17 - Phase B evidence plumbing lands: publication outcome, tombstone failure loud in evidence, the four D-063 prune phases, and pending expiry join grant-trace.v1

The conformance pilot's Phase B (brief section 6 table, task 5 of the v0.3.9 conclusion plan) needs evidence sources for publication, crash windows and prune. Seven tags join the v1 contract (12 through 18), all pre-publication extensions: no projector exists yet, so there is zero migration.

**Ruling 1 - publish_outcome and mark_published_failed close the publication boundary.** publish_outcome fires when dispatch decides to publish from the effect outcome (the fired path only; the refused path never attempts publication and already terminates in effect_refused, D-078). mark_published_failed fires inside the tombstone catch: D-080 ruling 1's promise realized, the failure stays fail-safe in control flow (D-061 at-least-once) but is preserved loud in evidence, and the projector must never map it to MarkPublished success. The grant remains a durable consumed, unpublished orphan until recovery; the kill-writes test drives exactly this window with a new crash-injection seam.

**Ruling 2 - the four D-063 prune phases are trace-visible, including the prune inside every commit.** commitConsumed runs pruneExpired first (D-061 ruling 4), so the rename phases (temp written, temp synced, renamed, reopened) interleave inside every grant commit trace. That is not noise: a crash mid-prune-during-commit is precisely the scenario D-063 fixed, and the last completed phase is now observable evidence. The prune events carry the ledger-path fingerprint, not a grant id: the prune is scoped to the log.

**Ruling 3 - expire_pending carries the collapse count in pc; a zero-count sweep is silent.** The intent table's expiry sweep (BE-GRANT-06a) emits after the transition returns, with the count. Restart/crash boundaries stay harness-owned (the brief's Crash/Restart epoch markers), and FinishExecuted/FinishFailed stay unbound until the pilot binds terminal intent completion; neither is a Phase A dependency.

**Ruling 4 - the crash-injection seam is test-only and fd-safe.** seamBreakLedgerWrites swaps the ledger's read-write handle for a read-only one on the same path: the next append fails (a write on an O_RDONLY fd is EBADF) exactly like a lost tombstone write, while every later close stays valid. Double-closing a real fd is a detected OS bug in std.Io.Threaded.closeFd, so the original handle leaks instead; harmless in a test process. Production never calls it. This is the deterministic stand-in for the SIGKILL crash injection the Phase B harness will run on its side.
...
## D-090 - 2026-08-22 - Audit path hardening: real chain validation without clock, causal revocation, subject-expiry wire field

**Date:** 2026-08-22
**Decision:** The historical/attestation path's declared debt is repaid in three parts: the audit certificate check becomes real, revocation positioning in audit becomes causal, and the Revoke body carries the subject's own prunable expiry.

**Ruling 1 - BE-HIST-01 is enforced by a chain split, not a fake timestamp.** `validateCertNoClock` returned success unconditionally (an expedient that validated nothing). binding.zig now splits validation into `validateCertChain` (roles, quorum, BE-REV-01 lifetime cap, CA signatures, trust set) plus the validity-window conjunct; `validateCert` = chain + window as before, and `validateCertNoClock` exposes exactly the chain with NO time parameter, so the type system proves no clock check can hide on the audit path. The narrow `CertChainError` return set cannot name `CertExpired`. `historicalValidity` runs this check on the AuditContext cert before any causal check: a committed signature survives its cert expiring, but a cert whose chain was never sound was never valid at any time.

**Ruling 2 - BE-HIST-04 audit positioning is causal, not blanket.** The old path rejected every envelope from a now-revoked sender because `ledger.isRevoked` returns only a bool. The ledger now exposes `getRevokeHash`, and the violation exists only when the audited envelope is a causal descendant of the revoke envelope (`dag.isAncestor(revoke_hash, env_hash)`). An envelope committed before its sender's revocation stays historically valid at commit time; admission keeps the immediate gate. Note recorded for the ledger: the original TODO suggested `supersedes(revoke, env, env)`, which would always return false because supersession demands STRICT self-ancestry of the claim node; the direct ancestor predicate is the correct instrument.

**Ruling 3 - The Revoke body carries the SUBJECT's expiry or nothing (fail-closed).** The F10 placeholder stored the ADMIN's certificate expiry as the revocation's prunable timestamp: unrelated data, fail-open if an admin cert expired before a still-live subject cert (early prune, then `isRevoked` false for a revoked key). New **BE-CTRL-03**: the Revoke body carries `u64be subject_cert_expiry_ms`; absent or short bodies mean NEVER pruned. The reader is the pure `verify.revokePruneExpiry`. No node substitutes another principal's expiry.

**Known coverage boundary, stated not hidden.** Six d090 mutants (re-clocking the audit path, skipping audit validation, dropping causal ancestry, blanket-rejection regression, hiding the revoke hash, zeroing the prune fallback) are wired into the mutation gate and killed 6/6. The admission-wiring call site itself has no behavioural mutant: no admission-level envelope fixture exists in this codebase's test architecture, and building one mid-slice was judged disproportionate for one line that calls a unit-tested reader. The regression risk there is covered by review against BE-CTRL-03's text, and stated here so nobody later mistakes the denominator for complete wiring coverage.

**What this seals.** v0.5.3 scope: items 2+4 of the post-seal debt list are closed (audit path weaker than admission path; verify.zig expiry placeholder). evidence.zig's hook pattern was inspected and is NOT debt: the three ResolveContext hooks are the designed seam awaiting the Utterance grammar deferred by RED-TEAM-09 F1; there is no production call site to wire yet. Suite 403 pass / 7 skip / 0 fail with the rogue-CA-through-audit test; chunk run d090: 6/6 killed, 0 survived. M2 receipt at final HEAD pending ONE full-suite run, per the standing discipline.

## D-091 - 2026-08-23 - D-090 closeout reconciliation: fmt canonicalization + budget compaction, no caps moved

**Date:** 2026-08-23
**Decision:** The D-090 slice's interrupted closeout left three mechanical debts the continuity gauntlet surfaced: fmt drift carried since the D-089 pilot commits (`listener.zig`, `daemon.zig`, `relay.zig` never canonicalized), an unmeasured M5/M11 budget breach (the CertChainError split grew `binding.zig` 190 to 220, putting session-state at 789/759 and the post-authentication unit at 1518/1500), and the fmt reflow's +3 landing `relay.zig` at 259/256.

**Ruling 1 - canonicalize, then compact, never re-floor.** BE-SURF-03's unit cap MUST NOT be raised, so a post-auth overage can only be resolved by removing real lines. The compaction is comment-tightening only: `binding.zig` 220 -> 190 restores the exact D-087 floor with zero code change (the `CertChainError` set, the `validateCertNoClock` wrapper, and the chain split are untouched; suite 403 passed / 7 skipped / 0 failed before and after), and `relay.zig` 259 -> 255 replaces the stale 510-line tripwire comment with the v0.3.5 cap sentence. No sub-unit cap moved, no file changed list, no SPEC text.

**Ruling 2 - an unrun gauntlet is an unmeasured slice.** The budget breach sat in five unpushed commits for ~3 hours next to a green-looking suite log because the previous session stopped before running prumo/fmt. The closeout gauntlet is the only moment the surface law is enforced. Standing order for every slice: code -> chunk kill -> full suite -> fmt+prumo+tests gauntlet -> docs sync -> receipt bump -> push.

**What this closes.** D-090's pending item (full-suite run at final HEAD) is discharged: 168/168 non-equivalent killed, 0 survived, 1 documented equivalent, receipt `tools/m2-mutation-receipt`. Budgets: pre-auth 1460/1500 (handshake 961/990, relay 255/256, listener 244/250), post-auth 1488/1500 (wire-parser 652/652, session-state 759/759, sync 77/89). M1 114/116 with BE-CTRL-03 and BE-TR-01a the two keyed tests still owed (worklist, not regressions). No wire bytes change.

## D-092 - 2026-08-23 - The v1.0 line: three external gates, zero features

**Date:** 2026-08-23
**Decision:** A v1.0 target exists from this entry onward, and it contains ZERO features. v1.0 is defined as the first version declared safe to carry real authority, and every gate on its line is external by construction. Code milestones end at the v0.6.0 seal (D-091 scope); the entire final stretch is work that cannot be produced by writing more src/. Rationale: a "final stretch" only means anything against a drawn line, and an agent-only roadmap can never draw one, because an agent can always write more code instead of facing outside judgement.

**Ruling 1 - G1, external adversarial exposure.** An independent review of the COMPOSITION (the primitives are standard; compositions are what kill protocols), seeded by `CRYPTO-REVIEW-BRIEF.md`, followed by a red-team window with real stakes. Findings are triaged against THREAT-MODEL claims in the open; silent fixes without published findings are forbidden. Independence is defined IN THIS GATE, in the vocabulary of the SPEC 11.4 shared-authorship disclaimer: the reviewer must not share an author, employer, funding source, or close collaboration history with any Bolina implementer - work from the same ecosystem is self-consistent evidence, not independence (the TLA+ model under 11.4 is the standing example of the difference). The 2026-08-20 review (`CRYPTO-REVIEW-FINDINGS.md`) does NOT satisfy G1 for exactly this reason: its author shares the implementation's ecosystem. It enters as PRE-AUDIT BASELINE for the external reviewer (brief + F1-F14 registry + dispositions), never as the review itself. The external review runs against a frozen tag (sealed v0.6.0), never against moving main. The gate closes on findings DISPOSITION - each finding fixed or accepted-with-name under SPEC 11 R3 discipline - not on "the review happened".

**Ruling 2 - G2, interop against frozen vectors.** The conformance vectors freeze at the v1.0-candidate seal. The MINIMUM that counts: one second independent implementation of the parser + verification chain (wire format, signatures carrying BE-SIG-01 tags, grant chain BE-GRANT-03) passing the frozen vectors, PLUS one live interop handshake between the two implementations over real transport. A codebase passing its own tests is not interop; self-play proves nothing about the wire. Full-protocol parity is BONUS, not gate - the SPEC 11.3 seal already records why ("not two full-protocol implementations"). Form note (owner ruling): the second implementation is structured as a reusable client library, Go first; it will be reused post-1.0, which changes the form, not the scope.

**Ruling 3 - G3, sustained adversarial soak.** At least 24h of continuous chaos + differential soak on dedicated hardware with zero unexplained failures. This discharges the deferred continuous component of SPEC 11.6 and inherits what 11.6 already demands of itself: parser coverage REPORTED (SPEC 11 R1 - the d089-d091 defence gates running un-wired from the exit code until found is the internal proof that measuring the harness happens at home too), measured config = shipped config (SPEC 11 R4, ReleaseSafe). "Zero unexplained failures" is already SPEC 11 R3 and stands as written.

**Ruling 4 - feature freeze is part of the line.** Explicitly post-1.0, nothing may join scope after this entry: C ABI, WASM target, multi-CA hierarchy, Utterance path (RED-TEAM-09 F1) - AND every finding accepted-with-name during disposition (example: 64-bit `scope_id` held with T11 as the argument). Accepted limitations freeze WITH the features; neither gets "improved" during the final stretch. The line stays fixed so it cannot recede into a moving horizon. Citation discipline (owner catch, 2026-08-23): this entry's internal rulings are cited as "D-092 ruling N", NEVER abbreviated "RN" - bare RN stays reserved for the SPEC 11 conformance rules (R1 measure-the-harness, R2 single-sided metrics forbidden, R3 cause-per-failure, R4 measured build = shipped build). The accepted-with-name freeze lives HERE, in D-092 ruling 4; it has nothing to do with SPEC 11 R4.

**Ownership note (transversal, per owner amendment).** Each gate carries an OWNER and a DEADLINE-OR-DECISION date recorded here, because all three depend on third parties and third-party work stagnates silently. G1 requires a reviewer chosen by the owners (who, when, cost); G2 requires a third party; G3 requires provisioned hardware. Bolina v1.0 cannot be sealed from inside this repository alone, and that is the point of the line: the remaining proof is precisely the kind an implementer cannot grant itself.

## D-093 - 2026-08-25 - M1 116/117: the two owed fixtures bound; the last unbound marker is an accepted trade, not a worklist item

**Date:** 2026-08-25
**Decision:** The M1-AUDIT worklist's two owed keyed tests (D-090's BE-CTRL-03 Revoke-body fixture, D-089's BE-TR-01a binding-layout fixture) are bound, raising M1 to 116/117 with the high-water ratchet at 116. The remaining unbound marker, BE-HIST-04a, is NOT a worklist item: it is the accepted-coupling audit disposition (SPEC 9.1, pre-audit v0.6.0 section 4) and stays unbound by design unless the owners decide to persist historical trust sets alongside channels.

**Ruling 1 - a binding is a literal fixture, not a rename.** The two F10_D090 tests already exercised revokePruneExpiry but carried no marker name and built their witness with writeInt, so the expectation agreed with itself by construction. The replacement tests hardcode the eight bytes 0x00 0x00 0x01 0x8B 0xCF 0xE5 0x68 0x00 against the independently written decimal 1_700_000_000_000 (D-027 discipline). BE_TR_01a applies the same discipline at the frame level: a 70-byte literal (u16be cert_len 4 | cert C711429A | 64-byte sig fill B0..EF) so the slice offsets are asserted, plus the three rejections (cert_len zero -> Malformed via .bind_cert_len_zero, short frame -> Truncated, trailing byte -> TrailingBytes via .bind_trailing). parseBindingMessage had zero direct unit tests before this entry; its only callers were the daemon's parse-then-drop path and the fuzz harness.

**Ruling 2 - test-only drift still invalidates the M2 receipt.** prumo's M2 drift check compares the receipt SHA's committed tree against HEAD over src/ and tools/mutation-test.py, and the *_test.zig files live under src/. A slice that changes only tests therefore still owes a full-suite re-run and a receipt bump at the new HEAD; the green M2 row mid-slice was real only because the edits were not yet committed. Standing sequence unchanged from D-091 ruling 2: code -> chunk kill -> full suite -> gauntlet -> docs sync -> receipt bump -> push.

**What this closes.** The M1-AUDIT worklist is empty: every marker that owes a keyed test has one. M1 116/117, high water 116. Tests 443 pass / 7 skip (450 total). No production line changed, no wire bytes, no surface movement (pre-auth 1460/1500, post-auth 1488/1500).

## D-094 - 2026-08-25 - G3 CLOSED: the 24h adversarial soak passed and its signed receipt is repository evidence; deliberately NO new prumo gate

**Date:** 2026-08-25
**Decision:** Gate G3 of D-092 is closed on operator-executed evidence: a single continuous 24h03 window (2026-08-24T20:10Z -> 2026-08-25T20:14Z) on dedicated owner hardware against frozen tag v0.6.1 (`7207fb0`), ReleaseSafe by pin. Results: 110 chaos units = 110 billion inputs with coverage ON, 0 panics; 36 differential rounds Zig-vs-Python oracle, 0 divergences; 0 infra failures; max 76 C (crit 100); parser exit coverage complete (zero unreached exits across all rounds, R1). The run was wrapped in Lastro from t0; the receipt verifies clean against the CI CA (`lastro verify`, clock-free chain per BE-HIST-01), resource `bol:a1c71dab…/git/bolina/7207fb0…/check/g3-soak-24h`. Evidence committed under `docs/receipts/g3/` (receipt, observed stream, hashes, co-tenancy journal, service snapshots, fuzz summary); `thermal.csv` stays off-repo by size, pinned by HASHES.sha256.

**Ruling 1 - why there is no M13 gate.** A gate guards a LIVING invariant that can regress; a completed soak is historical evidence that cannot un-happen. Requiring "the file exists" would be theater, and running Lastro inside prumo-verify would add a Go-toolchain dependency to the gauntlet. The living dependencies G3 left behind are already guarded: cert freshness by M12 (reissue alarm), toolchain identity by build.zig's ReleaseSafe pin. The G3 record lives here as a decision, not as an always-on check.

**Ruling 2 - the wrapper exit-1 is disclosed inside the evidence itself.** The signed stream records `[lastro] exit-status=1` while its own SUMMARY line shows 0/0/0; root cause (kit bug #5: hash step hit a vanished log under set -e before emitting exit 0) is preserved in the stream and fixed same-day in tools/g3-soak.sh together with the other five operator-reported defects (user-unit detection, sudo HOME shift, system-zig shadow R4 hazard, diff-workdir accumulation, phase log overwrite). Zero unexplained failures stands: every anomaly in the window has a named cause.

**Gate scoreboard after this entry.** G3 CLOSED. G2: Go codec + full grant verification chain landed at `loonix/lastro` `3e289a4` and was INDEPENDENTLY re-run by this repo's agent (39 top-level tests plus subtests, zero failures) - the second head now reproduces not just wire formats but authority decisions. Owed to close G2: one live Noise_IK interop handshake; runbook prepared at `docs/G2-INTEROP-HANDSHAKE.md` (staged A/B/C ladder with observable success criteria). G1 open, unlocked once G2 closes per owner sequencing; reviewer pack targets v0.6.1.

## D-095 - 2026-08-25 - G2 live interop caught a real wire defect vectors could never see; initial closure superseded by operator rectification, conformant re-run pending

**Decision:** Gate G2 of D-092 is closed on operator-executed evidence: a live Noise_IK handshake with the Go implementation as initiator against the real Zig daemon on dedicated hardware - identical transcript hash both sides, mutual BE-TR-01 binding verified clock-free, and one Intent envelope built, signed, and sealed by Go ADMITTED by the daemon state machine (`GET /v1/intents/<id>` returns `pending`). Lastro receipt VERIFIED against the CI CA at commit `a36d25f`. The second implementation now reproduces wire formats AND authority decisions over a real transport.

**The gate earned its keep.** The live handshake surfaced a conformance bug that neither mutation testing, nor fuzzing, nor the frozen vectors could see: the type-2 response carried its two session-index fields swapped relative to SPEC 4.1a (initiator echo at offset 4, responder slot at offset 8). Invisible internally because no Zig consumer parses those fields and test index numbering coincided with slot numbering; a spec-conformant initiator would reject every response where announced index != responder slot. This is precisely why Ruling 2 demands a live handshake: self-play proves nothing about the wire.

**Dispositions:** (1) Swap fixed on main to the conformant layout with a byte-level pin in the pilot e2e using a distinct announced index, kill-proven by reverting the fix under the pin; frozen tag v0.6.1 stays as-is, disclosed as known issue in the reviewer pack. (2) Runbook money-shot corrected: wire-path admission is proven by `GET /v1/intents/<id>` returning `pending`; `bolina_intents_admitted_total` counts only HTTP-path admissions (first live run found the original step unobservable for wire admits). (3) Type-4 transport addressing audited against 4.1a and found conformant: `receiver_index` carries the peer's session index, matching session.zig's documented contract. (4) Go side flips its two msg2 reads before the next interop run against post-fix main.

**Amendment (same day, operator-initiated).** The live-evidence receipt above was generated against the daemon WITH the index swap: the operator's Go client had worked around it by reading offset 8. It therefore proves the cryptographic handshake (identical transcript h, mutual binding) but NOT full-G2 conformance, and is superseded by its own author. Cross kill-proofs now exist from both ends: this repo's pilot pin kills the swapped layout under fix-revert (`expected 10948382, found 0`), and the operator's SPEC-conformant Go (lastro `35f02aa`) against the still-buggy daemon (`a36d25f`) mis-reads the responder slot, mis-addresses transport, gets dropped, and observes not-found - exactly the predicted impact for a conformant initiator. Closing procedure agreed: push the conformant daemon, operator rebuilds on the box, re-runs interop stage C expecting `pending`, regenerates the receipt against conformant code on BOTH sides; that VERIFIED closes G2. The superseded receipt is retained as evidence of the defect's real-world reach.

**Gate scoreboard after this entry (amended).** G3 CLOSED. G2 PENDING final verification: vectors byte-exact + grant chain verified, live handshake crypto proven, closure awaits the post-fix interop re-run receipt. G1 unlock fires on actual G2 closure; reviewer selection remains the owner's move.

**Amendment 2 (same day, independent verification).** Closure executed as agreed, and verified independently on the Bolina side. Attachment transfer failed twice on Telegram; the operator moved the evidence into git instead (lastro `ad26855`, `evidence/g2/`) which is the product's own thesis: receipts verify from the repo, not from files that get lost. Verification performed here: built lastro fresh from that commit and ran `lastro verify --cert ci/runner-g3/cert.bin --ca ci/ca/ca/ca0.pub --ca ci/ca/ca/ca1.pub evidence/g2/g2.receipt` -> VERIFIED; resource `bol:a1c71dab7e4c24a3/git/bolina/9447ca86cec2de0312311ab2af70937e7f2a7ff9/check/g2-interop-live`; digest `de91a82a1a43c585fa02ff2744f7ab0e8422344eeacdbfdf803e75dfd88fe39f`; executor `a1c71dab7e4c24a3`; chain validated clock-free (BE-HIST-01); observed 2026-08-25T22:48:02Z inside the cert window. Digest-to-stream binding confirmed byte-exact: BLAKE2s-256 of `g2-observed.log` equals the receipt digest exactly (verification note: first hash attempt used SHA-256 and mismatched; the receipt digest is Blake2s256 of the captured stream per `cmd/lastro/main.go:271`). The covered stream itself shows the conformant wire live: daemon slot `0x00000000` at offset 4 with initiator echo `0x51530001` (the fixed 4.1a layout), transcript h identical, mutual binding clock-free, stage C admission state `pending`, `[lastro] exit-status=0`. A copy of `g2.receipt` + `g2-observed.log` is snapshotted under `docs/receipts/g2/` (canonical source: lastro repo `evidence/g2/`).
