...
| T9 | Executor-local storage is honest and exclusive | The grant ledger's guarantees assume fsync semantics hold (no lying disk write-cache) and exactly one process owns the log (BE-EXEC-01, enforced since MD3: open() takes flock(LOCK_EX\|LOCK_NB) on the fd, re-acquired after pruneExpired's reopen, so a second instance sharing one grant-ledger path fails with the distinct error.Locked instead of interleaving positional appends at a stale eof and corrupting the log). |
