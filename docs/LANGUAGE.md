...
| Implementation lines | **490** capability slice (`parser.zig` 284, `verify.zig` 182, `main.zig` 13, `tests.zig` 11) + round-4 attestation layer (`evidence.zig` 258, `dag.zig` 190, `evidence_test.zig` 451, `dag_test.zig` 230, `evidence_test_helpers.zig` 202). Fuzz and vector harnesses excluded. Current state after the budget split (D-030): **two BE-SURF-03 units, each measured against its own 1500-line cap by the M5/M11 gates (`tools/prumo-verify`)** — pre-authentication **1460/1500**, subdivided per D-043 and SPEC v0.3.6 into a handshake sub-unit **961/990** (`mac.zig` 173, `noise.zig` 460, `parser.zig` 328), a relay sub-unit **255/256** (`relay.zig` 255; cap ratcheted DOWN from the declared 510 to its measured floor by SPEC v0.3.6, per D-058; canonical-form reflow and stale-tripwire comment cleanup at the D-090 closeout per D-091), and a listener sub-unit **244/250** (`listener.zig` 173, `handshake.zig` 71; cap 250 declared by SPEC v0.3.6 for the daemon Phase B surface, BE-EXEC-02/03 and BE-SESS-02); post-authentication **1488/1500**, subdivided per D-052 and D-054 (session-state re-floored and sync rebalanced by D-087) into a wire-parser sub-unit **652/652** (cap ratcheted DOWN from 723 to its measured floor by D-054), a session-state sub-unit **759/759** (`binding.zig` 190, `parser/channel.zig` 399, `parser/session.zig` 253, `reassembly.zig` 240, `replay.zig` 114, `session.zig` 215; cap re-floored from 748 to its measured 759 by D-087 after the F1 kex_pubkey binding fix; D-090's CertChainError split grew binding.zig to 220 and the D-091 closeout compaction restored the 190 floor, comment-tightening only, tests unchanged), and a sync sub-unit **77/89** (`parser/sync.zig` 77, cap rebalanced from 100 by D-087 so the sub-cap sum stays 1500; cap sum 652 + 759 + 89 = 1500, nothing else raised, per BE-SURF-03); the split lands on BE-SURF-01's authentication boundary, and the pre-split single-unit reading plus the headroom estimate are in `NOISE-SESSION-ESTIMATE.md`, the sync budget history in `SYNC-ESTIMATE.md`. Non-budgeted implementation files outside both units: `verify.zig` 711 (Grant verifier BE-GRANT-03, now consulting the durable revocation set at checks 3-4 per D-064 and owning its checks 6-9 state via intent/sender table references per D-087, plus the channel, mesh, envelope-admission, and refusal verify layers, and the D-090 `revokePruneExpiry` reader), `intent.zig` 216 (pending-intent state machine, non-surface per D-052), `resolver.zig` 322 (canonical resource resolution BE-RES-01..06 and the resolve-then-admit gate, non-surface per D-052), `render.zig` 56 (approving view BE-GRANT-07/07a, non-surface per D-052), `sync.zig` 289 (backfill engine BE-SYNC-01..05, non-surface per D-054), `relay_store.zig` 143 (store-and-forward engine BE-MESH-03, non-surface per D-058), `dispatch.zig` 339 (Phase A dispatch core + Phase D durable seam: body_type routing onto the bound machines with the daemon seams as injected hooks, the check-11 consumed-grant hook widened to carry the durable commit context and tombstoned after the effect, the isRevokedHook fail-safe seam binding the grant-path revocation consult to the durable ledger per D-064, and the F13 taxonomy gate passing intent/sender tables to verify per D-087, non-surface per D-059/D-062/D-064), `grant_ledger.zig` 554 (Phase D durable consumed-grant + revocation two-phase append log over `std.Io` with fsync barrier, orphan recovery, and pruneExpired crash-safe by atomic rename per D-063, non-surface per D-061/D-063), `relay_serve.zig` 216 (Phase C daemon serve-loop: live UDP recv/send glue tying the listener, session table, and relay engine into a packet path with store-and-forward drain, non-surface per D-059/D-060, BE-EXEC-04), `ledger.zig` 333 and `historical.zig` 105 (ledger store and historical audit path, non-surface per D-045/D-047; D-090 gave the ledger `getRevokeHash` for the causal audit and historical the `validateCertNoClock` chain check), `evidence.zig` 294 and `dag.zig` 190 (attestation), `adversarial_audit.zig` 147 (the §11.5 R2 post-hoc effect-log auditor, D-066), `grant_trace.zig` 163 (comptime-gated trace projector for the ZIG-TLA pilot, D-076), `keys.zig` 193 and `daemon.zig` 342 (node key material and node core, D-089), `main.zig` 178, `tests.zig` 39; dedicated `*_test.zig` files total 11118; instrumentation and fuzz harness (`coverage.zig` 173, `fuzz.zig` 455) excluded. |
...
| Exit-point coverage (SPEC §11.6) | **71 of the 72 enumerated parser exit points reached** over a bounded run of 4,000,000 inputs (88,000,000 parser calls, 0 panics), measured 2026-08-11 at HEAD after D-057 wired all 22 parse entry points into the harness. One exit is unreached: `data_payload_oversize`, which needs a type-4 transport packet whose payload exceeds 1384 bytes while its type byte and three reserved bytes still parse, a shape neither the seed lineage nor random bytes construct at this rate. The previous reading was **17 of 72**, measured 2026-08-08 when the harness called only six entry points; the 55-exit gap named there closed by wiring the transport, mesh, certificate, relay, refusal, and sync parsers rather than by moving the denominator, which is still the 72-member `Branch` enum derived mechanically via `@typeInfo` (`src/coverage.zig`). Measured by hand-instrumented counters, reproduced with `zig build coverage -Dcoverage -Dfuzz-budget=4000000`. Native toolchain coverage has no script-readable output on this toolchain, so manual instrumentation is the measurement, not a proxy (residual in `THREAT-MODEL.md` §4.6). Corpus: 22 seeds, one per parse entry point (envelope, intent, grant, span, effect, claim, refusal from `test/vectors.json`; a two-signature certificate and thirteen others synthesized from their SPEC field tables; the binding message built around the real vectors certificate), 5 mutation operators (bit flip, byte overwrite, truncate, saturate, extend), 40% mutated-seed / 60% fully-random, 4096-byte input cap. This is the chaos-mode reading, which feeds every input to all 22 entry points; the differential oracle's bounded merge-gate corpus reaches 69 of 72 on its own tagged routing (M4, D-057). |
| Mutation kill | **168/168 mutants killed** (full non-chunked suite re-run 2026-08-23 at the D-090 closeout, receipt `tools/m2-mutation-receipt`, log `logs/mutation_final_d090.log`, the D-090 audit-path defence domain added to the set; history: 162/162 at the v0.5.2 seal HEAD `7fcc336` (the d089 CHECK-ABSENCE defence family, receipt `logs/launch_final_7fcc336.out`), 160/160 at M2 receipt HEAD `91f05f7` receipt `tools/m2-mutation-receipt` (the D-086 scope mutants added to the v21 set), v21 155/155 measured 2026-08-13 at F4 HEAD `bd00e6d`, receipt `logs/mutation_f4_full.log`, v20 151/151 at Phase-D HEAD `df4ceef`, v19 144/144 at Phase-C HEAD `7456655`, v18 138/138 at Phase-B HEAD `fe35fab`, v17 131/131 at Phase-A HEAD `a804117`, v16 125/125 at mesh-slice HEAD `3ef57c7`, v15 118/118 at sync-slice HEAD `8055801`, v14 106/106 at render-slice HEAD `5c255f3`, v13 100/100 at resolver HEAD `4917390`): 17 on the Grant verifier (14 over the 12 modelled BE-GRANT-03 checks, since check 10's three bounds are attacked separately, plus 3 on the BE-GRANT-03b callback property and its ordering against the expiry and ledger checks), 11 on the attestation layer (the nine section-7 properties keyed to the resolution record and supersession), 4 on the transport DoS constants (all four section-4 markers: mac1-label, cookie-rotate, window-bits, max-message), 11 on the session phase (the six section-4 session markers: key-schedule, mac1-first, nonce-counter, rekey-bound, rekey-zero, binding-sig), 8 on the channel layer (the seven section-6 markers: BE-CHAN-01/02, BE-GEN-01/03/04, BE-CTRL-01/02), 6 on the mesh layer (the four section-5 markers: BE-MESH-01/04/05/06), 13 on the mesh store-and-forward path (the five section-5 markers now including BE-MESH-03: seven mutants over `src/relay_store.zig` and `src/relay.zig` attacking the per-recipient packet quota, the 2048-byte body cap, the 72-hour TTL purge on the caller clock, oldest-first drain order, byte-for-byte opacity of the stored body, the drain rewrite of the relay-layer recipient_index at registration, and no-service for unknown recipients extending BE-MESH-04 to storage), 11 on the relay layer (the eight §5.2a properties: route and registration formats, the signed-span boundary, unknown-recipient drop, table bound, skew bound, expiry pruning, and the 0x07 domain tag), 13 on the ledger layer (the nine §9/BE-ENV properties over `src/ledger.zig` and `src/historical.zig`: unknown-parent rejection, hash-only commitment fidelity, count advance, equivocation divergence in both directions, window enforcement including the forbidden strict maximum, anchor and revocation immutability with signer binding, causal descent on audit, and approver gating of Grant and Refusal), 6 on the pending-intent state machine (the six section-8.2 BE-GRANT markers over `src/intent.zig`: restart collapse, resource exclusivity, the T_pending timeout sweep, intent_id dedupe, the refusal state transition, and REJECTED terminality), 3 on the refusal verifier (the three section-8.5 properties over `src/verify.zig` `verifyRefusalThen`: the 0x06 domain tag, the approver role, and the applyRefusal hook), and 10 on the resolver (the six §8.4 properties over `src/resolver.zig`: the BLAKE2s-256 first-8-bytes fingerprint and its lowercase-hex rendering, the grammar walk rejecting dot and dotdot segments, unknown and ambiguous resolution refused, the foreign-fingerprint refusal, declared-alias collapse onto one canonical, and the signed resource-set verification with big-endian length encoding), and 6 on the render view (the two §8.3 properties over `src/render.zig`: the digest recomputed from exactly the displayed action bytes, attacked at the source (resource-id digesting), the value (zero-pin), and the extent (action truncation), and the untrusted-rationale discipline, attacked at the label, the field order, and the optionality of the primary content), and 12 on the sync layer (the five §6.4 properties over `src/sync.zig` and `src/parser/sync.zig`: session/member/not-revoked admission, the min(max_envelopes,64) and 1 MiB response bounds with the exact truncated flag, the depth-128 total-4096 walk budget with its no-retry discipline, the serve-8 issue-4 rate window in both directions, and verify-before-adopt), and 6 on the Phase A dispatch seam (the five D-059 properties over `src/dispatch.zig`: body_type routing onto the bound machines, the BE-ENV-02 envelope signature gate, the subject cert riding the cert_for_sender seam keyed by the intent sender, the EXECUTING transition after the grant frame, and the consumed-grant check-and-set commit), and 7 on the daemon listener/handshake surface (the three §0.4 properties over `src/listener.zig` and `src/handshake.zig`: BE-EXEC-02 attacked at the registry ownership check and at the OS-level bind refusal, BE-EXEC-03 attacked at the address-family length gate and at the socket family itself (the ipv4-listener-on-AF_INET6 mutant, killed by the getsockname family witness; the sockaddr family byte is an equivalent mutant because macOS bind ignores it), and BE-SESS-02 attacked at the initiation type/size gate, at the swallowed-handshake-failure path, and at the committed session's send/recv key direction), and 6 on the daemon relay-serve path (the five BE-EXEC-04 properties over `src/relay_serve.zig`: the live forward arm to a known recipient endpoint, the store-on-unknown-recipient deferral, the drain-on-late-registration rewrite, the no-service gate for an unregistered recipient, and the byte-for-byte forwarded body), and 7 on the durable grant-ledger surface (the seven D-061 properties over `src/grant_ledger.zig`: the durable commit row appended and fsynced before the effect (BE-GRANT-01), the single-shot isConsumed replay gate (BE-GRANT-01), orphan detection of a committed grant whose published tombstone never landed (BE-GRANT-01a), the at-least-once re-emission of an un-tombstoned orphan (BE-GRANT-01a), durable revocation that survives restart (BE-REV-02), pruneExpired dropping expired consumed grants while keeping live ones (BE-EXEC-01), and the discard of a partial trailing record from a crash mid-write before the fsync), and 4 on the F4 grant-revocation seam (the three D-064 properties over `src/verify.zig` and `src/dispatch.zig`: check 3 approver revocation consult attacked by absence and by inversion, check 4 subject revocation consult attacked by absence, and the isRevokedHook no-ledger fail-safe attacked by flipping the orelse default), and 3 on the d089 defence family (the three D-089 CHECK-ABSENCE properties: MD3 the exclusive flock at open over `src/grant_ledger.zig` whose removal lets a second writer silently share the log, MD4 the post-expiry compaction over `src/intent.zig` whose removal leaks dead slots until TableFull forever, and MD5 the re-registration dedup over `src/relay.zig` whose removal appends duplicate routes until the table fills), and 6 on the D-090 audit-path defence domain (the three D-090 properties over `src/historical.zig`, `src/binding.zig`, `src/verify.zig`, and `src/ledger.zig`: the BE-HIST-01 clock check leaking into the NoClock audit path so committed signatures are re-clocked, historicalValidity skipping certificate validation entirely, BE-HIST-04 causal ancestry dropped so post-revocation envelopes pass audit, BE-HIST-04 regressed to blanket rejection so pre-revocation envelopes fail audit, getRevokeHash never exposing the revoke hash so causal audit is impossible, and the prune expiry falling back to zero instead of never-prune so a revocation is forgotten immediately). All nineteen check sets are derived at run time, fifteen from `SPEC.md`, the grant set from the enumerated 0-11 list and the conformance sentence, the evidence set from the §7 tables and BE-EVID markers, the transport and session sets from the bold BE-TR markers, and the channel, mesh, relay, ledger, intent, refusal, resolver, render, and sync sets from the section-6, section-5, §5.2a, §9, §8.2, §8.5, §8.4, §8.3, and §6.4 markers, the daemon set from the bold BE-EXEC-02/03 and BE-SESS-02 markers of §0.4, the relay_serve set from the bold BE-EXEC-04 marker of §0.4, and the dispatch set from the recorded D-059 ruling (Phase A dispatch code carries no SPEC markers), and the grant_revocation set from the recorded D-064 ruling (the F4 seam carries no SPEC markers), and the d089 and d090 defence sets from the recorded D-089 and D-090 rulings (the defence seams carry no SPEC markers), none of them stated by the harness. The window-bits mutant SURVIVED against the symbolic tests and was killed only after `replay_test.zig` and `reassembly_test.zig` were rewritten to literal values; the same D-027 rule drove the session KATs, which are literal bytes from an independent Python blake2s/hmac run. The ledger round's first full re-run surfaced one survivor on the grant side: BE-REV-01's new 30-day cap rejected the check-4 role-swapped subject fixture before the role check ran, so the role mutant was unfalsifiable on that witness; retightening that fixture to the PRIVILEGED_CERT window restored the kill (98cf577, D-049). The resolver round's first full run started on a tree carrying residue from a killed launch (a grant-domain mutant left un-restored in `src/verify.zig`): that mutant was skipped and one resolver survivor was masked until the tree was restored and the clean re-run killed all 100. The harness restores from launch-time snapshots, so a killed launch can poison the next run: verify zero residue before launching. Multi-file mutation results produced before `7553d21` (which restored only the current target between mutants, so a kill could be credited to a leftover mutant from a different file) were re-run under full mutant isolation; runs are chunked by a `MUTATION_DOMAIN` env filter so each domain stays under the timeout ceiling and the restore `finally` always fires (D-035); v10 keyed the relay sub-unit's eight §5.2a properties with eleven mutants over `src/relay.zig`, v11 keyed the ledger's nine §9/BE-ENV properties with thirteen mutants over `src/ledger.zig` and `src/historical.zig`, v12 keyed the pending-intent state machine's six §8.2 BE-GRANT markers over `src/intent.zig` and the refusal verifier's three §8.5 properties over `src/verify.zig`, and v13 keys the resolver's six §8.4 properties with ten mutants over `src/resolver.zig`, and v14 keys the render view's two §8.3 properties with six mutants over `src/render.zig`, and v15 keys the sync layer's five §6.4 properties with twelve mutants over `src/sync.zig` and `src/parser/sync.zig`, and v16 keys BE-MESH-03 with seven mutants over `src/relay_store.zig` and `src/relay.zig`, the marker entering the denominator after its v10 deferral (D-051 closed by Daniel's 2026-08-11 ruling, mechanics warranted by D-058), and v17 keys the Phase A dispatch seam's five D-059 properties with six mutants over `src/dispatch.zig`, and v18 keys the daemon listener/handshake surface's three §0.4 properties with seven mutants over `src/listener.zig` and `src/handshake.zig`, and v19 keys the daemon relay-serve path's five BE-EXEC-04 properties with six mutants over `src/relay_serve.zig`, and v20 keys the durable grant-ledger surface's seven D-061 properties with seven mutants over `src/grant_ledger.zig` and re-anchors the three seam mutants (two verify check-11 inversions/orderings, one dispatch consumed-commit) to the widened check-11 hook `(grant_id, not_after, now)` and the durable `consumedHook` (D-062), and v21 keys the F4 grant-revocation seam's three D-064 properties with four mutants over `src/verify.zig` and `src/dispatch.zig` (RED-TEAM-10 closeout; the witnesses ride `src/dispatch_test.zig`). The grant denominator read 7 rather than 12 until 2026-08-07: SPEC's conformance sentence still recorded checks 3, 4, 6, 7 and 8 as delegated to the executor after task 7 had folded them into the routine, so five enforced checks sat outside the measured set. Repairing the sentence raised the denominator and those five checks now carry mutants (D-039). |
...
the corpus and 71 of 72 exit points, where the first merge reported neither. That gap closed by wiring every parse entry
point into the harness (D-057), not by moving the denominator: the transport, mesh, certificate, relay, refusal, and sync
parsers now carry their own seeds and their own tagged routing, and they are additionally replayed against an independent
Python reference on every merge (M4). One exit stays unreached, `data_payload_oversize`, and it is named on the gate row
every run rather than rounded away.
